...
typedef struct ndb_gpu_backend
{
    ...
    /* Launchers */
    int  (*launch_l2_distance)(const float *A, const float *B, float *out,
                               int n, int d, ndb_stream_t stream);
    int  (*launch_cosine)(const float *A, const float *B, float *out,
                          int n, int d, ndb_stream_t stream);
    int  (*launch_kmeans_assign)(const float *X, const float *C, int *idx,
                                 int n, int d, int k, ndb_stream_t stream);
    int  (*launch_kmeans_update)(const float *X, const int *idx, float *C,
                                 int n, int d, int k, ndb_stream_t stream);
    int  (*launch_quant_fp16)(const float *in, void *out, int n,
                              ndb_stream_t stream);
    int  (*launch_quant_int8)(const float *in, int8_t *out, int n,
                              float scale, ndb_stream_t stream);
    int  (*launch_quant_binary)(const float *in, uint8_t *out, int n,
                                ndb_stream_t stream);
    int  (*launch_pq_encode)(const float *X, const float *codebooks,
                             uint8_t *codes, int n, int d, int m, int ks,
                             ndb_stream_t stream);
