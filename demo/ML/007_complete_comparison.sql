...
SELECT 
    ...
    is_deployed,
    deployed_at,
    'Recently deployed' as deployment_age
...
SELECT 
    project_name,
    latest_version as versions,
    latest_algorithm as algorithm,
    latest_samples as samples_trained,
    latest_training_time || 'ms' as latest_time,
    avg_training_time || 'ms' as avg_time,
    CASE 
        WHEN deployed_version IS NOT NULL THEN 'YES (v' || deployed_version || ')'
        ELSE 'NO'
    END as deployed
...
SELECT 
    ...
    COUNT(*) as num_models,
    CASE 
        WHEN algorithm = 'minibatch_kmeans' THEN 'RECOMMENDED: Fastest, scalable'
        WHEN algorithm = 'kmeans' THEN 'GOOD: Reliable, well-tested'
        WHEN algorithm = 'gmm' THEN 'ADVANCED: Probabilistic clustering'
        WHEN algorithm = 'outlier' THEN 'SPECIALIZED: Anomaly detection'
        ELSE 'TESTING'
...
SELECT 
    ...
    SUM(m.num_samples) as total_samples_trained
