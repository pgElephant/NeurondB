...
SELECT 
	...
	ROUND(COALESCE((m.metrics::jsonb->>'n_components')::numeric, 0), 6) AS n_components
...
SELECT
	...
	CASE 
		...
		WHEN (m.metrics::jsonb ? 'silhouette_score')
			THEN ROUND((m.metrics::jsonb ->> 'silhouette_score')::numeric, 6)::text
		ELSE 'N/A' 
	END AS value
...
SELECT 
	...
	CASE 
		WHEN m.metrics IS NULL THEN NULL
		WHEN (m.metrics::jsonb ? 'error') THEN NULL
		WHEN (m.metrics::jsonb ? 'silhouette_score') THEN ROUND((m.metrics::jsonb->>'silhouette_score')::numeric, 6)
		ELSE NULL 
	END AS final_silhouette
FROM gpu_metrics_temp m;

-- Store results in test_metrics table
INSERT INTO test_metrics (
	test_name, algorithm, model_id, train_samples, test_samples,
	silhouette_score, inertia, n_clusters, updated_at
)
SELECT 
	'013_gmm_basic',
	'gmm',
...
WHERE m.model_id = (SELECT model_id FROM test_metrics WHERE test_name = '013_gmm_basic')
...
WHERE tm.test_name = '013_gmm_basic';
...
SELECT 
	...
	tm.train_samples,
	tm.test_samples,
	ROUND(tm.silhouette_score::numeric, 6) AS silhouette_score,
