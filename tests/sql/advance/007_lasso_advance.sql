...
-- Exhaustive detailed test for lasso: all train, error, predict, evaluate.
...
\echo '=========================================================================='
\echo 'lasso: Exhaustive GPU/CPU + Error Coverage (1000 rows sample)'
\echo '=========================================================================='
...
FROM cpu_model_temp_007;

\echo 'Test 3: Custom hyperparameters (alpha=0.5, fit_intercept=true)'
DROP TABLE IF EXISTS custom_model_temp_007;
CREATE TEMP TABLE custom_model_temp_007 AS
SELECT neurondb.train('lasso', 
	'test_train_view', 
	'features', 'label', 
...
FROM custom_model_temp_007;

\echo 'Test 4: Different alpha value (alpha=2.0)'
DROP TABLE IF EXISTS alpha2_model_temp_007;
CREATE TEMP TABLE alpha2_model_temp_007 AS
SELECT neurondb.train('lasso', 
	'test_train_view', 
	'features', 'label', 
...
DROP TABLE IF EXISTS alpha01_model_temp_007;
CREATE TEMP TABLE alpha01_model_temp_007 AS
SELECT neurondb.train('lasso', 
	'test_train_view', 
	'features', 'label', 
...
BEGIN
	BEGIN
		PERFORM neurondb.train('lasso','missing_table','features','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('lasso','test_train_view','notafeat','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('lasso','test_train_view','features','notalabel','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('lasso','test_train_view','features','label','{"alpha":-1.0}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('lasso','test_train_view',NULL,'label','{}'::jsonb);
...
FROM (
	SELECT neurondb.predict((SELECT gpu_model_id FROM gpu_model_temp_007 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	ROUND(MAX(score)::numeric, 4) AS max_score
FROM (
	SELECT neurondb.predict((SELECT cpu_model_id FROM cpu_model_temp_007 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	neurondb.predict((SELECT custom_model_id FROM custom_model_temp_007 LIMIT 1), features) AS prediction
FROM test_test_view
LIMIT 1;
...
SELECT 
	...
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT custom_model_id FROM custom_model_temp_007 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 100
) b;
...
SELECT 
	...
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT alpha01_model_id FROM alpha01_model_temp_007 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 50
) b;
...
BEGIN
	SELECT cpu_model_id INTO cpu_mid_temp FROM cpu_model_temp_007 LIMIT 1;
	BEGIN
...
DECLARE
	...
	cpu_mid integer;
BEGIN
	SELECT cpu_model_id INTO cpu_mid FROM cpu_model_temp_007 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT gpu_model_id INTO gpu_mid FROM gpu_model_temp_007 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT custom_model_id INTO custom_mid FROM custom_model_temp_007 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT alpha01_model_id INTO alpha01_mid FROM alpha01_model_temp_007 LIMIT 1;
...
FROM neurondb.ml_models m, gpu_model_temp_007 t
...
SELECT 
	...
	CASE 
		...
		ELSE format('%s bytes', pg_column_size(m.model_data))
	END AS model_data_status
FROM neurondb.ml_models m, cpu_model_temp_007 t
...
SELECT 
	...
	m.metrics->>'r_squared' AS r_squared,
	m.metrics->>'mse' AS mse
FROM neurondb.ml_models m, custom_model_temp_007 t
...
WHERE algorithm = 'lasso'
GROUP BY algorithm;

\echo ''
\echo '=========================================================================='
\echo '✓ lasso: Full exhaustive code-path test complete (1000-row sample)'
\echo '=========================================================================='
