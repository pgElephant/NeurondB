...
import { MiddlewareManager, MCPRequest, MCPResponse } from "./middleware.js";
...
class NeurondbMCPServer {
	...
	private mlTools: MLTools;
	private analyticsTools: AnalyticsTools;
	private ragTools: RAGTools;
	private projectTools: ProjectTools;
	private gpuTools: GPUTools;
	private quantizationTools: QuantizationTools;
	private dimensionalityTools: DimensionalityTools;
	private driftTools: DriftTools;
	private metricsTools: MetricsTools;
	private hybridTools: HybridTools;
	private rerankingTools: RerankingTools;
	private indexingTools: IndexingTools;
	private dataManagementTools: DataManagementTools;
		...
		this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
			...
			const response = await this.middleware.execute(mcpRequest, async () => {
				try {
					...
					this.logger.error(`Tool execution error: ${name}`, error as Error, { args });
					return {
						content: [
							{
								type: "text",
	...
	private async handleBuiltInTool(name: string, args: any): Promise<any> {
		const features = this.config.getFeaturesConfig();

		switch (name) {
			case "vector_search":
				if (!features.vector?.enabled) throw new Error("Vector features are disabled");
			...
			case "train_ml_model":
				if (!features.ml?.enabled) throw new Error("ML features are disabled");
				return this.trainMLModel(args);
			case "predict_ml_model":
				if (!features.ml?.enabled) throw new Error("ML features are disabled");
				return this.mlTools.predict(args);
			case "cluster_data":
				if (!features.analytics?.enabled) throw new Error("Analytics features are disabled");
				return this.clusterData(args);
			case "detect_outliers":
				if (!features.analytics?.enabled) throw new Error("Analytics features are disabled");
				return this.analyticsTools.detectOutliersZScore(args);
			case "rag_chunk_text":
				if (!features.rag?.enabled) throw new Error("RAG features are disabled");
				return this.ragTools.chunkText(args);
			case "rerank_results":
				if (!features.rag?.enabled) throw new Error("RAG features are disabled");
				return this.rerankResults(args);
			case "create_ml_project":
				if (!features.projects?.enabled) throw new Error("Project features are disabled");
			...
			case "gpu_cluster_kmeans":
				return this.gpuTools.clusterKMeansGPU(
					args.table,
					args.vector_column,
					args.k,
					args.max_iter
			...
			case "gpu_hnsw_search":
				return this.gpuTools.hnswSearchGPU(
					args.table,
					args.vector_column,
					args.query_vector,
					args.limit
			...
			case "train_pq_codebook":
				return this.quantizationTools.trainPQCodebook(
					args.table,
					args.vector_column,
					args.num_subvectors,
					args.num_centroids,
			...
			case "train_opq_codebook":
				return this.quantizationTools.trainOPQCodebook(
					args.table,
					args.vector_column,
					args.num_subvectors,
					args.num_centroids,
			...
			// Dimensionality Reduction Tools
			case "reduce_pca":
				return this.dimensionalityTools.reducePCA(
					args.table,
					args.vector_column,
					args.target_dimensions
				);
			case "whiten_embeddings":
				return this.dimensionalityTools.whitenEmbeddings(
					args.table,
					args.vector_column
				);
			...
			case "detect_centroid_drift":
				return this.driftTools.detectCentroidDrift(
					args.baseline_table,
			...
			case "detect_distribution_divergence":
				return this.driftTools.detectDistributionDivergence(
					args.baseline_table,
					args.baseline_column,
					args.current_table,
					args.current_column,
			...
			// Metrics Tools
			case "recall_at_k":
				return this.metricsTools.recallAtK(
					args.ground_truth_table,
			...
			case "precision_at_k":
				return this.metricsTools.precisionAtK(
					args.ground_truth_table,
					args.ground_truth_column,
					args.predictions_table,
					args.predictions_column,
			...
			case "f1_at_k":
				return this.metricsTools.f1AtK(
					args.ground_truth_table,
					args.ground_truth_column,
					args.predictions_table,
					args.predictions_column,
			...
			case "mean_reciprocal_rank":
				return this.metricsTools.meanReciprocalRank(
					args.ground_truth_table,
					args.ground_truth_column,
					args.predictions_table,
					args.predictions_column,
			...
			case "clustering_metrics":
				return this.metricsTools.clusteringMetrics(
					args.table,
					args.vector_column,
					args.cluster_column,
					args.metric
			...
			case "hybrid_search_fusion":
				return this.hybridTools.hybridSearchFusion(
					args.semantic_table,
					args.lexical_table,
					args.id_column,
					args.semantic_score_column,
					args.lexical_score_column,
					args.alpha
				);
			case "ltr_rerank":
				return this.hybridTools.ltrRerankPointwise(
					args.query,
					args.documents,
					args.features
				);
			...
			// Reranking Tools
			case "mmr_rerank":
				return this.rerankingTools.mmrRerank(
					args.table,
					args.query_vector,
					args.vector_column,
					args.lambda,
			...
			case "rerank_cross_encoder":
				return this.rerankingTools.rerankCrossEncoder(
					args.query,
					args.documents,
					args.model,
					args.top_k
			...
			case "rerank_llm":
				return this.rerankingTools.rerankLLM(
					args.query,
					args.documents,
					args.model,
					args.top_k
			...
			case "create_ivf_index":
				return this.indexingTools.createIVFIndex(
					args.table,
					args.vector_column,
					args.index_name,
					args.num_lists
			...
			case "rebalance_index":
				return this.indexingTools.rebalanceIndex(
					args.index_name,
					args.threshold
				);
			...
			// Data Management Tools
			case "vacuum_vectors":
				return this.dataManagementTools.vacuumVectors(
					args.table,
					args.dry_run
				);
			case "compress_cold_tier":
				return this.dataManagementTools.compressColdTier(
					args.table,
					args.days_threshold
				);
			case "sync_index_async":
				return this.dataManagementTools.syncIndexAsync(
					args.index_name,
					args.replica_host
				);
	...
	private async trainMLModel(args: {
		table: string;
		feature_col: string;
		label_col: string;
		algorithm: string;
	...
	private async clusterData(args: {
		table: string;
	...
	private async rerankResults(args: {
		query: string;
		documents: string[];
		method?: string;
		model?: string;
