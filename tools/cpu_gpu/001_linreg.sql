...
-- Example: python ml_dataset.py adult
...
SELECT
	AVG(POWER(neurondb.predict(m.model_id, features) - label, 2))::float8 AS mse,
	SQRT(AVG(POWER(neurondb.predict(m.model_id, features) - label, 2)))::float8 AS rmse,
	AVG(ABS(neurondb.predict(m.model_id, features) - label))::float8 AS mae
