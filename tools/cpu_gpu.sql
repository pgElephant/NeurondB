...
\echo '   Creating 1,000,000 samples with 256-dim float32 vectors.'
CREATE TABLE neurondb_cpu_gpu_data AS
SELECT
    gs AS sample_id,
    (
        SELECT array_agg((random() * 2 - 1)::float4 ORDER BY dim)::vector(256)
...
\echo 'Step 3: CPU training (GPU disabled)...'
...
\echo 'Step 4: GPU training (enable NeuronDB GPU acceleration)...'
