...
rerank_get_candidates(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		funcctx->user_fctx = SPI_copytupletable(SPI_tuptable);
...
rerank_index_warm(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < nqueries; i++)
	{
		...
		{
			SPI_finish();
			elog(ERROR, "Failed to cache candidates for query_hash %lu: %s", qhash, sql.data);
		}
		resetStringInfo(&sql);
	}
