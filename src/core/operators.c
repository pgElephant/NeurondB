...
vector_avg(PG_FUNCTION_ARGS)
{
	...
	{
		bool isnull = false;
		Datum vectdat = array_ref(input, 1, &i, -1, -1, false, 'd', &isnull);
...

		{
			...
			if (!sum_allocated)
			{
				size_t bytes = VARHDRSZ + sizeof(int32) + sizeof(float4) * vec->dim;
				sumvec = (Vector*) palloc0(bytes);
				SET_VARSIZE(sumvec, bytes);
				sumvec->dim = vec->dim;
				memcpy(sumvec->data, vec->data, sizeof(float4)*vec->dim);
				sum_allocated = true;
			}
			else
			{
...
vector_contains(PG_FUNCTION_ARGS)
{
	...
	{
		bool isnull2 = false;
		Datum d2 = array_ref(set2, 1, &i2, -1, -1, false, 'd', &isnull2);
		if (isnull2)
			continue;

		Vector *v2 = DatumGetVector(d2);
		if (!v2)
			continue;
		...
		{
			bool isnull1 = false;
			Datum d1 = array_ref(set1, 1, &i1, -1, -1, false, 'd', &isnull1);
			if (isnull1)
				continue;
			Vector *v1 = DatumGetVector(d1);
			if (!v1 || v1->dim != v2->dim)
				continue;
			found = true;
			for (d = 0; d < v1->dim; d++)
...
vec_join(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		snprintf(querybuf, sizeof(querybuf),
		         ...
		         left_table, right_table, join_pred);
		...
		if (spi_ret != SPI_OK_CONNECT)
		{
			...
			if (left_table)  pfree(left_table);
			if (right_table) pfree(right_table);
			if (join_pred)   pfree(join_pred);
			PG_RETURN_NULL();
		}
	...
	while (state->current < state->ntuples)
	{
		...
		left_id    = SPI_getbinval(tuple, state->tuptable->tupdesc, 1, &isnull[0]);
		right_id   = SPI_getbinval(tuple, state->tuptable->tupdesc, 2, &isnull[1]);
		left_vec_d = SPI_getbinval(tuple, state->tuptable->tupdesc, 3, &isnull[2]);
		right_vec_d= SPI_getbinval(tuple, state->tuptable->tupdesc, 4, &isnull[3]);
...

		{
			...
			rettup = heap_form_tuple(funcctx->tuple_desc, values, nulls);
...
graph_knn(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		text *graph_col_text = PG_GETARG_TEXT_PP(1);
		...
		elog(DEBUG1, "Graph KNN: query_dim=%d, max_hops=%d, k=%d", query_vec->dim, max_hops, k);
		char querybuf[2048];
		...
		snprintf(querybuf, sizeof(querybuf),
		         "SELECT id, vector, %s FROM nodes", graph_col_cstr);
	...
	while (state->current < state->ntuples)
	{
		...
		tuple = state->tuptable->vals[state->current++];
		id = SPI_getbinval(tuple, state->tuptable->tupdesc, 1, &isnull1);
		vector_d = SPI_getbinval(tuple, state->tuptable->tupdesc, 2, &isnull2);
		...
		if (item_vec != NULL && query_vec != NULL && item_vec->dim == query_vec->dim)
		{
			dist = 0.0f;
			for (j = 0; j < item_vec->dim; j++)
			{
				float4 tmp = item_vec->data[j] - query_vec->data[j];
				dist += tmp * tmp;
			}
			dist = sqrtf(dist);
		}
...
hybrid_rank(PG_FUNCTION_ARGS)
{
	...
	char    *rel_str = NULL, *txt_str = NULL;
	...
	if (query_vec->dim <= 0)
		ereport(ERROR, (errmsg("query vector dimension must be positive")));

	elog(DEBUG1, "Hybrid rank: relation=%s, query_dim=%d, text_len=%zu",
		 text_to_cstring(relation_name), query_vec->dim, strlen(text_to_cstring(query_text)));
	...
	if (SPI_connect() == SPI_OK_CONNECT)
	{
		...
		appendStringInfo(&sql,
			"SELECT alpha, beta FROM neurondb_hybrid_weights WHERE relation = '%s'",
			rel_str);
		...
		{
			bool isnull1 = false, isnull2 = false;
			float4 aval = DatumGetFloat4(SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull1));
			float4 bval = DatumGetFloat4(SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull2));
			if (!isnull1)
				alpha = aval;
			if (!isnull2)
				beta = bval;
...
vec_window_rank(PG_FUNCTION_ARGS)
{
	...
	/* Validate ref_vector */
	if (ref_vector->dim <= 0)
		ereport(ERROR, (errmsg("reference vector dimension must be positive")));
...
vec_route(PG_FUNCTION_ARGS)
{
	...
	for (i = 1; i <= nshards; ++i)
	{
		...
		cent_dat = array_ref(shard_centroids, 1, &i, -1, -1, false, 'd', &isnull);
		if (isnull)
			continue;
