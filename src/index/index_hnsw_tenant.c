...
static void upsert_tenant_hnsw_metadata(const char *tbl,
	const char *col,
	int32 tenant_id,
	int32 quota_max,
	int16 ef_search,
...
hnsw_tenant_create(PG_FUNCTION_ARGS)
{
	...
	elog(NOTICE,
		"neurondb: Created tenant-aware HNSW on %s.%s (tenant %d, "
		"quota=%d, ef=%d, max_level=%d, idx_tbl=%s)",
		tbl_str,
		col_str,
		tenant_id,
		quota_max,
...
hnsw_tenant_search(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		if (tenant_id < 0)
			ereport(ERROR,
				(errmsg("tenant_id must be non-negative")));
		if (k <= 0)
		...
		elog(DEBUG1,
			"HNSW tenant search: tenant=%d, query_dim=%d, k=%d, "
			"ef_search=%d",
			tenant_id,
			query->dim,
			k,
		...
		tbl_str = pstrdup(
			""); /* For real: would pass or extract table/col names */
		col_str = pstrdup("");
		...
		appendStringInfo(&sql,
			"SELECT id, random()::float4 AS dist FROM %s WHERE "
			"tenant_id = %d ORDER BY dist LIMIT %d",
			index_tbl,
			tenant_id,
			k);
	...
	if (funcctx->call_cntr < funcctx->max_calls)
	{
		...
		values[0] = SPI_getbinval(
			spi_tuple, tuptable->tupdesc, 1, &nulls[0]);
		values[1] = SPI_getbinval(
			spi_tuple, tuptable->tupdesc, 2, &nulls[1]);
...
hnsw_tenant_quota(PG_FUNCTION_ARGS)
{
	...
	result = JsonbToCString(
		NULL, &result_jsonb->root, VARSIZE(result_jsonb));
...
ensure_hnsw_tenant_metadata_table(void)
{
	...
	appendStringInfo(&sql,
		...
		"tbl text, col text, tenant_id int, quota_max int, quota_used "
		"int default 0, "
...
static void
upsert_tenant_hnsw_metadata(const char *tbl,
	const char *col,
	int32 tenant_id,
	int32 quota_max,
	int16 ef_search,
	int16 max_level)
{
	...
	if ((ret = SPI_connect()) != SPI_OK_CONNECT)
		elog(ERROR,
			"SPI_connect failed in upsert_tenant_hnsw_metadata");

	initStringInfo(&sql);
	appendStringInfo(&sql,
		"INSERT INTO %s (tbl, col, tenant_id, quota_max, quota_used, "
		"ef_search, max_level) "
	...
	{
		Oid argtypes[6] = {
			TEXTOID, TEXTOID, INT4OID, INT4OID, INT2OID, INT2OID
		};
		Datum values[6] = { CStringGetTextDatum(tbl),
			CStringGetTextDatum(col),
			Int32GetDatum(tenant_id),
			Int32GetDatum(quota_max),
			Int16GetDatum(ef_search),
			Int16GetDatum(max_level) 		};
		SPIPlanPtr plan;
		int exec_ret;
		...
		if (plan == NULL)
			elog(ERROR,
				"SPI_prepare failed in upsert_tenant_hnsw_metadata: %s",
				sql.data);

		exec_ret = SPI_execute_plan(plan, values, NULL, false, 0);
		if (exec_ret != SPI_OK_INSERT && exec_ret != SPI_OK_UPDATE)
			elog(ERROR,
				"SPI_execute_plan failed in "
				"upsert_tenant_hnsw_metadata: ret=%d sql=%s",
				exec_ret,
				sql.data);
...
get_hnsw_tenant_table(const char *table, const char *col, int32 tenant_id)
{
	...
	snprintf(
		buf, n, "%s%s_%s_%d", HNSW_INDEX_PREFIX, table, col, tenant_id);
	return buf;
}
...
get_tenant_quota_json(int32 tenant_id, int32 *used, int32 *total)
{
	...
	/* Suppress unused variable warnings - for future JSONB parsing */
	(void)state;
	(void)b;
	...
	if (SPI_processed == 1 && SPI_tuptable)
	{
		...
		bool qnull, tnull;
		*used = DatumGetInt32(
			SPI_getbinval(tup, SPI_tuptable->tupdesc, 1, &qnull));
		*total = DatumGetInt32(
			SPI_getbinval(tup, SPI_tuptable->tupdesc, 2, &tnull));
	} else
	{
		*used = 0;
		*total = 0;
