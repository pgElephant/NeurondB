...
auto_route_query(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0))
        ereport(ERROR, (errmsg("auto_route_query: input query text is NULL")));
    if (PG_ARGISNULL(1))
        ereport(ERROR, (errmsg("auto_route_query: embedding_length is NULL")));
    ...
    if (query_str == NULL)
        ereport(ERROR, (errmsg("auto_route_query: failed to convert query text to C string")));
    ...
    elog(DEBUG1, "neurondb:auto_route_query: route=%s embedding_length=%d", use_ann ? "ANN" : "FTS", embedding_length);
...
learn_from_query(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(1))
        ereport(ERROR, (errmsg("learn_from_query: actual_recall is NULL")));
    if (PG_ARGISNULL(2))
        ereport(ERROR, (errmsg("learn_from_query: latency_ms is NULL")));
    ...
    if (query_str == NULL)
        ereport(ERROR, (errmsg("learn_from_query: failed to convert query to C string")));
    ...
    elog(DEBUG1, "neurondb:learn_from_query: fprint=%llu recall=%.6f latency=%d", (unsigned long long)fingerprint, actual_recall, latency_ms);

    /* SPI connect - recover/exit safely */
    if (SPI_connect() != SPI_OK_CONNECT)
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("learn_from_query: SPI_connect failed")));
    ...
    {
        SPI_finish();  /* clean up! */
        ereport(ERROR, (errmsg("learn_from_query: cannot create neurondb_query_history")));
    }
    ...
    /* Check for existing fingerprint */
    {
        ...
        appendStringInfo(&sql,
            ...
            "WHERE fingerprint = %llu", (unsigned long long)fingerprint);
    ...
    if (found)
    {
        ...
        int32 ef_search = 0;
        int32 beam_size = 0;
        bool isnull1 = false, isnull2 = false, isnull3 = false;

        seen_count = DatumGetInt32(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 2, &isnull1));
        ef_search = DatumGetInt32(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 3, &isnull2));
        beam_size = DatumGetInt32(
            SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 4, &isnull3));
        ...
        if (isnull1 || isnull2 || isnull3)
        {
            ...
            if (query_str)
                pfree(query_str);
            ereport(ERROR, (errmsg("learn_from_query: NULL detected in stats fields")));
        }

        seen_count = (seen_count < 0) ? 1 : seen_count + 1; /* avoid underflow */
        ...
        /* Update the tuple defensively */
        {
            ...
            appendStringInfo(&usql,
                ...
                "SET last_recall=%.6f, last_latency=%d, seen_count=%d, "
                "ef_search=%d, beam_size=%d WHERE fingerprint=%llu",
                actual_recall, latency_ms, seen_count, ef_search, beam_size, (unsigned long long)fingerprint);

            if (SPI_execute(usql.data, false, 0) != SPI_OK_UPDATE)
            {
                if (usql.data) pfree(usql.data);
                SPI_finish();
                if (query_str)
                    pfree(query_str);
                ereport(ERROR, (errmsg("learn_from_query: failed to UPDATE neurondb_query_history")));
            }
            if (usql.data) pfree(usql.data);
        }
    }
    else
    {
        ...
        appendStringInfo(&isql,
            ...
            "(fingerprint, last_recall, last_latency, seen_count, ef_search, beam_size) "
            "VALUES (%llu, %.6f, %d, %d, %d, %d)",
            (unsigned long long)fingerprint, actual_recall, latency_ms, 1, 32, 8);

        if (SPI_execute(isql.data, false, 0) != SPI_OK_INSERT)
        {
            if (isql.data) pfree(isql.data);
            SPI_finish();
            if (query_str)
                pfree(query_str);
            ereport(ERROR, (errmsg("learn_from_query: failed to INSERT neurondb_query_history")));
        }
        if (isql.data) pfree(isql.data);
    }
...
scale_precision(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0))
        ereport(ERROR, (errmsg("scale_precision: input vector is NULL")));
    if (PG_ARGISNULL(1))
        ereport(ERROR, (errmsg("scale_precision: memory_pressure is NULL")));
    if (PG_ARGISNULL(2))
        ereport(ERROR, (errmsg("scale_precision: recall_target is NULL")));
    ...
    elog(DEBUG1, "neurondb:scale_precision: target=%dbit (mem=%.3f recall=%.3f)", target_precision, memory_pressure, recall_target);

    /* Defensive instantiation of output vector */
    result = new_vector(input->dim);
    if (result == NULL)
        ereport(ERROR, (errmsg("scale_precision: failed to allocate result vector")));
    ...
    if (target_precision == 8)
    {
        ...
        if (!int8buf)
            ereport(ERROR, (errmsg("scale_precision: failed to alloc int8 buffer")));
...
prefetch_entry_points(PG_FUNCTION_ARGS)
{
    ...
    if (PG_ARGISNULL(0))
        ereport(ERROR, (errmsg("prefetch_entry_points: index_name is NULL")));
    if (PG_ARGISNULL(1))
        ereport(ERROR, (errmsg("prefetch_entry_points: query_vector is NULL")));
    ...
    if (!idx_str)
        ereport(ERROR, (errmsg("prefetch_entry_points: index_name conversion failed")));
    ...
    if (SPI_connect() != SPI_OK_CONNECT)
    {
        if (idx_str)
            pfree(idx_str);
        ereport(ERROR, (errmsg("prefetch_entry_points: SPI_connect failed")));
    }

    /* Query index statistics -- limit all dynamic stats lookups, always deallocate SQL buffers */
    {
        StringInfoData sql;
        int           ret;
        initStringInfo(&sql);
        ...
        ret = SPI_execute(sql.data, true, 10);
        if (ret == SPI_OK_SELECT && SPI_processed > 0)
        {
            ...
            for (q = 0; q < (int)SPI_processed; q++)
            {
                ...
                pagesDatum = SPI_getbinval(SPI_tuptable->vals[q], SPI_tuptable->tupdesc, 2, &isnull);
                if (!isnull)
                {
    ...
    elog(DEBUG1, "neurondb:prefetch_entry_points: prefetched_count=%d", prefetched_count);
