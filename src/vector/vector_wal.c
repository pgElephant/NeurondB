...
parse_vector_str(const char *vec, int *out_dim)
{
	...
	*out_dim = 0;
	...
	while (isspace((unsigned char)*copy)) copy++;
	if (copy[0] == '[')
		copy++;
	ptr = copy + strlen(copy) - 1;
	while (ptr > copy && isspace((unsigned char)*ptr)) ptr--;
	if (ptr >= copy && *ptr == ']') *ptr = '\0';
		...
		if (endptr == token || isnan(val) || isinf(val))
		{
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
					 errmsg("vector contains invalid float8 value: \"%s\"", token)));
		}

		if (count >= capacity)
		{
			capacity *= 2;
			result = (float8 *) repalloc(result, capacity * sizeof(float8));
		}
...
vector_wal_compress(PG_FUNCTION_ARGS)
{
	...
	if (dim != base_dim)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("vector and base_vector dimensions do not match (%d vs %d)", dim, base_dim)));

	initStringInfo(&compressed);

	elog(DEBUG1, "neurondb: Compressing vector using delta encoding, dim=%d", dim);
	...
	while (i < dim)
	{
		...
		while ((i + run_len < dim) &&
			   fabs((cur_values[i + run_len] - base_values[i + run_len]) - delta) < DBL_EPSILON)
		{
			run_len++;
		}

		/* Write run-length, delta pair */
		appendStringInfo(&compressed, "%d:%.17g%s", run_len, delta, (i + run_len < dim) ? ";" : "");
		i += run_len;
	}
...
vector_wal_decompress(PG_FUNCTION_ARGS)
{
	...
	elog(DEBUG1, "neurondb: Decompressing vector from delta encoding: \"%s\"", comp_str);
	...
	if (*p != 'D')
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("Compressed vector format error: missing dimension header")));
	...
	if (found_dim <= 0 || *endptr != ':')
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			 errmsg("Compressed vector format error: malformed dimension: \"%s\"", p)));
	dim = found_dim;
	p = endptr + 1;
	...
	if (base_dim != dim)
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("base_vector dim %d does not match compressed dim %d", base_dim, dim)));
	...
	while (*p && out_idx < dim)
	{
		...
		if (run <= 0 || *endptr != ':')
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("Compressed vector format error at position %d: expected <run>:<delta>", (int)(p - comp_str))));
		p = endptr + 1;
		/* read delta */
		...
		if ((endptr == p) || isnan(delta) || isinf(delta))
			ereport(ERROR,
					(errcode(ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE),
					 errmsg("Compressed encoding contains invalid float8 delta: \"%s\"", p)));
		p = endptr;
		/* apply run times */
		for (int j = 0; j < run; ++j)
		{
			if (out_idx >= dim)
				ereport(ERROR,
						(errcode(ERRCODE_ARRAY_SUBSCRIPT_ERROR),
						 errmsg("Decoded vector length exceeds expected %d", dim)));
		...
		if (*p == ';')
			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("Compressed vector format error after delta at position %d: got '%c'", (int)(p - comp_str), *p)));
	}
	if (out_idx != dim)
		ereport(ERROR,
				(errcode(ERRCODE_ARRAY_SUBSCRIPT_ERROR),
				 errmsg("Decompression failed: output vector length %d does not match dimension %d", out_idx, dim)));
...
vector_wal_estimate_size(PG_FUNCTION_ARGS)
{
	...
	vec_str = text_to_cstring(vector); /* Postgres returns "" for zero-length text */
	...
	estimated_compressed_size = (int32) ((float4) original_size / compression_ratio);

	if (estimated_compressed_size < 0)
		estimated_compressed_size = 0;

	elog(DEBUG1, "neurondb: Estimated compression: %d -> %d bytes (%.1fx)",
		 original_size, estimated_compressed_size, compression_ratio);
...
vector_wal_set_compression(PG_FUNCTION_ARGS)
{
	...
	if (PG_ARGISNULL(0))
		ereport(ERROR,
				(errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
				 errmsg("enable/disable argument must not be NULL")));
...
vector_wal_get_stats(PG_FUNCTION_ARGS)
{
	...
	if (total_bytes_compressed == 0)
		...
		compression_ratio = (float4)total_bytes_original / (float4)total_bytes_compressed;
	...
	elog(DEBUG1, "neurondb: WAL compression stats: %.2fx ratio (original=" NDB_INT64_FMT ", compressed=" NDB_INT64_FMT ")",
		 compression_ratio, NDB_INT64_CAST(total_bytes_original), NDB_INT64_CAST(total_bytes_compressed));
