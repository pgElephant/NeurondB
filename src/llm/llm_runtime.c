...
static void record_llm_stats(const char *model_name,
			      ...
			      const char *error_type)
{
	...
	ret = SPI_execute("SELECT 1 FROM pg_tables WHERE schemaname = 'neurondb' AND tablename = 'neurondb_llm_stats'", true, 0);
	...
	SPI_execute_with_args(
		"INSERT INTO neurondb.neurondb_llm_stats (model_name, total_requests, successful_requests, failed_requests, cache_hits, total_latency_ms, total_tokens, total_tokens_in, total_tokens_out, last_request_at, last_updated) "
		"VALUES ($1, 1, "
		...
		"  successful_requests = neurondb_llm_stats.successful_requests + CASE WHEN $3 THEN 1 ELSE 0 END, "
		"  failed_requests = neurondb_llm_stats.failed_requests + CASE WHEN $3 THEN 0 ELSE 1 END, "
		"  cache_hits = neurondb_llm_stats.cache_hits + CASE WHEN $4 THEN 1 ELSE 0 END, "
		"  total_latency_ms = neurondb_llm_stats.total_latency_ms + $5, "
		"  total_tokens = neurondb_llm_stats.total_tokens + $6 + $7, "
		"  total_tokens_in = neurondb_llm_stats.total_tokens_in + $6, "
		"  total_tokens_out = neurondb_llm_stats.total_tokens_out + $7, "
		"  last_request_at = now(), "
		"  last_updated = now()",
	...
	if (!success && error_type)
	{
		...
		ret = SPI_execute("SELECT 1 FROM pg_tables WHERE schemaname = 'neurondb' AND tablename = 'neurondb_llm_errors'", true, 0);
		if (ret == SPI_OK_SELECT && SPI_processed > 0)
		{
			SPI_execute_with_args(
				"INSERT INTO neurondb.neurondb_llm_errors (model_name, operation, error_type, latency_ms, error_timestamp) "
				"VALUES ($1, $2, $3, $4, now())",
				4, error_argtypes, error_values, error_nulls, false, 0
			);
		}
	}

	/* Record latency histogram */
	ret = SPI_execute("SELECT 1 FROM pg_tables WHERE schemaname = 'neurondb' AND tablename = 'neurondb_histograms'", true, 0);
	if (ret == SPI_OK_SELECT && SPI_processed > 0)
	{
		...
		appendStringInfo(&metric_name, "llm_%s_%s_latency_ms", operation ? operation : "unknown", model_name ? model_name : "unknown");
		...
		SPI_execute_with_args(
			"INSERT INTO neurondb.neurondb_histograms (metric_name, bucket_start, bucket_end, count, last_updated) "
			"VALUES ($1, $2, $3, $4, now()) "
			"ON CONFLICT (metric_name, bucket_start) DO UPDATE SET "
			"  count = neurondb_histograms.count + $4, "
			"  last_updated = now()",
			4, hist_argtypes, hist_values, hist_nulls, false, 0
		);
...
Datum ndb_llm_complete(PG_FUNCTION_ARGS)
{
    ...
    if (cache_lookup_text(keyhex.data, &cached))
    {
        ...
        success = true;
        {
            ...
            int32 token_length;
            int32 *token_ids = neurondb_tokenize_with_model(prompt, 2048, &token_length, cfg.model);
            ...
            PG_TRY();
            {
                ...
                int32 *output_token_ids = neurondb_tokenize_with_model(cached, 2048, &output_token_length, cfg.model);
        ...
        record_llm_stats(cfg.model, "complete", success, cache_hit, latency_ms, tokens_in, tokens_out, NULL);
    ...
    if (!llm_acquire_token())
    {
        ...
        if (neurondb_llm_fail_open)
        {
            ...
            record_llm_stats(cfg.model, "complete", success, cache_hit, latency_ms, tokens_in, tokens_out, error_type);
            PG_RETURN_NULL();
        }
        record_llm_stats(cfg.model, "complete", success, cache_hit, latency_ms, tokens_in, tokens_out, error_type);
...
Datum ndb_llm_embed(PG_FUNCTION_ARGS)
{
    ...
    if (cfg.endpoint && strncmp(cfg.endpoint, "mock://", 7) == 0)
    {
        ...
        tokens_in = 0;
        tokens_out = 0;
        record_llm_stats(cfg.model, "embed", success, cache_hit, latency_ms, tokens_in, tokens_out, NULL);
    ...
    if (!llm_acquire_token())
    {
        ...
        {
            ereport(WARNING, (errmsg("neurondb: LLM rate limited - fail open")));
            record_llm_stats(cfg.model, "embed", success, cache_hit, latency_ms, tokens_in, tokens_out, error_type);
            PG_RETURN_NULL();
        }
        record_llm_stats(cfg.model, "embed", success, cache_hit, latency_ms, tokens_in, tokens_out, error_type);
        ereport(ERROR, (errmsg("neurondb: LLM rate limited")));
    ...
    if (ndb_llm_route_embed(&cfg, &call_opts, txt, &vec, &dim) != 0 || dim <= 0)
    {
        end_time = GetCurrentTimestamp();
        latency_ms = (int64) ndb_elapsed_ms(start_time, end_time);
        cache_hit = false;
    ...
    success = true;
    {
        ...
        int32 token_length;
        int32 *token_ids = neurondb_tokenize_with_model(txt, 2048, &token_length, cfg.model);
        if (token_ids && token_length > 0)
            tokens_in = token_length;
        else
            tokens_in = 0;	/* Fallback: estimate from word count */
...
neurondb_llm_gpu_info(PG_FUNCTION_ARGS)
{
	...
	if (funcctx->call_cntr < 1)
	{
		...
		{
			backend_name = backend->name ? pstrdup(backend->name) : pstrdup("unknown");
...
ndb_llm_complete_batch(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		num_prompts = ArrayGetNItems(ARR_NDIM(prompts_arr), ARR_DIMS(prompts_arr));
		if (num_prompts <= 0)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("prompts array must not be empty")));

		/* Get params */
		params_in = (PG_NARGS() > 1 && !PG_ARGISNULL(1)) ? PG_GETARG_TEXT_PP(1) : NULL;
		params = params_in ? text_to_cstring(params_in) : "{}";

		/* Extract prompts from array */
		prompts = (char **) palloc0(num_prompts * sizeof(char *));
		for (i = 0; i < num_prompts; i++)
		{
			Datum elem = array_ref(prompts_arr, 1, &i, -1, -1, false, 'i', NULL);
			if (!DatumGetPointer(elem))
				continue;
			prompts[i] = text_to_cstring((text *) DatumGetPointer(elem));
		}
		...
		rc = ndb_llm_route_complete_batch(&cfg, &call_opts, (const char **) prompts,
						 num_prompts, params, &batch_resp);
		if (rc != 0)
		{
			/* Free prompts */
			for (i = 0; i < num_prompts; i++)
			...
			ereport(ERROR,
					(errcode(ERRCODE_EXTERNAL_ROUTINE_INVOCATION_EXCEPTION),
					 errmsg("neurondb: batch completion failed")));
		}

		/* Store state */
		{
			BatchCompleteState *state = (BatchCompleteState *) palloc0(sizeof(BatchCompleteState));
			state->num_prompts = num_prompts;
			state->current_idx = 0;
			state->batch_resp = (NdbLLMBatchResp *) palloc0(sizeof(NdbLLMBatchResp));
			*state->batch_resp = batch_resp;
	...
	attinmeta = funcctx->attinmeta;
	{
		BatchCompleteState *state = (BatchCompleteState *) funcctx->user_fctx;

		if (state->current_idx < state->num_prompts)
		{
			int idx = state->current_idx;
			NdbLLMBatchResp *resp = state->batch_resp;

			values[0] = Int32GetDatum(idx);
			if (resp->texts && resp->texts[idx])
			{
				values[1] = CStringGetTextDatum(resp->texts[idx]);
				values[2] = Int32GetDatum(resp->tokens_in ? resp->tokens_in[idx] : 0);
				values[3] = Int32GetDatum(resp->tokens_out ? resp->tokens_out[idx] : 0);
				values[4] = Int32GetDatum(resp->http_status ? resp->http_status[idx] : 200);
				nulls[0] = false;
				nulls[1] = false;
				nulls[2] = false;
				nulls[3] = false;
		...
		else
		{
			...
			if (state->batch_resp)
			{
				...
				{
					for (i = 0; i < state->batch_resp->num_items; i++)
						if (state->batch_resp->texts[i])
							pfree(state->batch_resp->texts[i]);
...
ndb_llm_rerank_batch(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		num_queries = ArrayGetNItems(ARR_NDIM(queries_arr), ARR_DIMS(queries_arr));
		if (num_queries <= 0)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("queries array must not be empty")));

		/* Get documents array (2D array) */
		documents_arr = PG_GETARG_ARRAYTYPE_P(1);
		if (ARR_NDIM(documents_arr) != 2)
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("documents_array must be a 2D array")));

		/* Get model */
		model_in = (PG_NARGS() > 2 && !PG_ARGISNULL(2)) ? PG_GETARG_TEXT_PP(2) : NULL;
		...
		{
			Datum elem = array_ref(queries_arr, 1, &i, -1, -1, false, 'i', NULL);
			if (!DatumGetPointer(elem))
				continue;
			queries[i] = text_to_cstring((text *) DatumGetPointer(elem));
		}

		/* Extract documents from 2D array */
		ndocs_array = (int *) palloc0(num_queries * sizeof(int));
		docs_array = (const char ***) palloc0(num_queries * sizeof(char **));
		for (i = 0; i < num_queries; i++)
		{
			int ndocs = ARR_DIMS(documents_arr)[1];
			char **docs;
			...
			{
				int indices[2] = {i, j};
				Datum elem = array_ref(documents_arr, 2, indices, -1, -1, false, 'i', NULL);
				if (!DatumGetPointer(elem))
					continue;
				docs[j] = text_to_cstring((text *) DatumGetPointer(elem));
			}
		...
		scores = NULL;
		nscores = NULL;
		rc = ndb_llm_route_rerank_batch(&cfg, &call_opts, (const char **) queries,
					       docs_array, ndocs_array, num_queries,
					       &scores, &nscores);
		if (rc != 0)
		{
			/* Free queries */
			...
			ereport(ERROR,
					(errcode(ERRCODE_EXTERNAL_ROUTINE_INVOCATION_EXCEPTION),
					 errmsg("neurondb: batch reranking failed")));
		}

		/* Store state */
		{
			BatchRerankState *state = (BatchRerankState *) palloc0(sizeof(BatchRerankState));
	...
	attinmeta = funcctx->attinmeta;
	{
		BatchRerankState *state = (BatchRerankState *) funcctx->user_fctx;
		...
		while (state->current_query_idx < state->num_queries)
		{
			...
			if (doc_idx < state->ndocs_array[query_idx])
			{
				if (state->scores && state->scores[query_idx])
				{
					...
					values[2] = Float4GetDatum(state->scores[query_idx][doc_idx]);
					nulls[0] = false;
					nulls[1] = false;
					nulls[2] = false;
				}
