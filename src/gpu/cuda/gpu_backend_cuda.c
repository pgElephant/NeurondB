...
static int  ndb_cuda_launch_l2_distance(const float *A, const float *B,
                                        float *out, int n, int d,
                                        ndb_stream_t stream);
static int  ndb_cuda_launch_cosine(const float *A, const float *B,
                                   float *out, int n, int d,
                                   ndb_stream_t stream);
static int  ndb_cuda_launch_kmeans_assign(const float *vectors,
                                          const float *centroids,
                                          int *assignments,
                                          int num_vectors, int dim, int k,
                                          ndb_stream_t stream);
static int  ndb_cuda_launch_kmeans_update(const float *vectors,
                                          ...
                                          int num_vectors, int dim, int k,
                                          ndb_stream_t stream);
static int  ndb_cuda_launch_quant_fp16(const float *input, void *output,
                                       int count, ndb_stream_t stream);
static int  ndb_cuda_launch_quant_int8(const float *input, int8_t *output,
                                       int count, float scale,
                                       ndb_stream_t stream);
static int  ndb_cuda_launch_quant_binary(const float *input, uint8_t *output,
                                         int count, ndb_stream_t stream);
static int  ndb_cuda_launch_pq_encode(const float *vectors,
                                      const float *codebooks,
                                      uint8_t *codes,
                                      int nvec, int dim, int m, int ks,
                                      ndb_stream_t stream);
...
ndb_cuda_init(void)
{
    ...
    {
        elog(WARNING, "neurondb: cudaGetDeviceCount failed: %s (devices=%d)",
    ...
    if (err != cudaSuccess)
    {
        elog(WARNING, "neurondb: cudaSetDevice(%d) failed: %s",
             cuda_ctx.device_id, cudaGetErrorString(err));
        return -1;
    }
    ...
    {
        elog(WARNING, "neurondb: cublasCreate failed with status %d", status);
        return -1;
    }
...
static int
ndb_cuda_launch_l2_distance(const float *A, const float *B, float *out,
                            int n, int d, ndb_stream_t stream)
{
    ...
    if (cudaMemcpyAsync(d_A, A, bytes, cudaMemcpyHostToDevice, native) != cudaSuccess)
        goto fail;
    if (cudaMemcpyAsync(d_B, B, bytes, cudaMemcpyHostToDevice, native) != cudaSuccess)
        goto fail;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (cublasScopy(cuda_ctx.handle, d, d_Ai, 1, d_diff, 1) != CUBLAS_STATUS_SUCCESS)
            goto fail;
        if (cublasSaxpy(cuda_ctx.handle, d, &alpha, d_Bi, 1, d_diff, 1) != CUBLAS_STATUS_SUCCESS)
            goto fail;
        if (cublasSnrm2(cuda_ctx.handle, d, d_diff, 1, &out[i]) != CUBLAS_STATUS_SUCCESS)
            goto fail;
    }
...
static int
ndb_cuda_launch_cosine(const float *A, const float *B, float *out,
                       int n, int d, ndb_stream_t stream)
{
    cudaStream_t native = stream ? (cudaStream_t) stream : 0;
    float *d_A = NULL;
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (cublasSdot(cuda_ctx.handle, d, d_Ai, 1, d_Bi, 1, &dot) != CUBLAS_STATUS_SUCCESS)
            goto fail;
        if (cublasSnrm2(cuda_ctx.handle, d, d_Ai, 1, &norm_a) != CUBLAS_STATUS_SUCCESS)
            goto fail;
        if (cublasSnrm2(cuda_ctx.handle, d, d_Bi, 1, &norm_b) != CUBLAS_STATUS_SUCCESS)
            goto fail;
...
static int
ndb_cuda_launch_quant_fp16(const float *input, void *output, int count,
                           ndb_stream_t stream)
{
    cudaStream_t native = stream ? (cudaStream_t) stream : 0;

    if (!cuda_ctx.initialized || input == NULL || output == NULL || count <= 0)
        return -1;

    return (launch_quantize_fp32_to_fp16(input, output, count, native) == cudaSuccess) ? 0 : -1;
}

static int
ndb_cuda_launch_quant_int8(const float *input, int8_t *output, int count,
                           float scale, ndb_stream_t stream)
{
    cudaStream_t native = stream ? (cudaStream_t) stream : 0;
    ...
    return (launch_quantize_fp32_to_int8(input,
                                         (signed char *) output,
                                         count,
                                         scale,
                                         native) == cudaSuccess) ? 0 : -1;
}

static int
ndb_cuda_launch_quant_binary(const float *input, uint8_t *output, int count,
                             ndb_stream_t stream)
{
    cudaStream_t native = stream ? (cudaStream_t) stream : 0;
    ...
    return (launch_quantize_fp32_to_binary(input,
...
static int
ndb_cuda_launch_kmeans_assign(const float *vectors, const float *centroids,
                              int *assignments, int num_vectors, int dim, int k,
                              ndb_stream_t stream)
{
    (void) stream;
...
static int
ndb_cuda_launch_kmeans_update(const float *vectors, const int *assignments,
                              float *centroids, int num_vectors, int dim, int k,
                              ndb_stream_t stream)
{
    int32_t *assign32;
    int32_t *counts;
    ...
    rc = gpu_kmeans_update(vectors, assign32, centroids, counts,
                           num_vectors, k, dim);
...
static int
ndb_cuda_launch_pq_encode(const float *vectors, const float *codebooks,
                          uint8_t *codes, int nvec, int dim, int m, int ks,
                          ndb_stream_t stream)
{
    (void) stream;
    ...
    return gpu_pq_encode_batch(vectors, codebooks, codes, nvec, dim, m, ks) == 0 ? 0 : -1;
}
