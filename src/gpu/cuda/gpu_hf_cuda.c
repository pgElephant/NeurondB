...
/*
 * ndb_cuda_hf_embed
 *	  Generate text embeddings using CUDA-accelerated Hugging Face model.
 *
 * This implementation:
				  ...
				  char **errstr)
{
	...
	if (entry == NULL || !entry->loaded)
	{
		...
		/* Copy weights to parent context with validation */
		{
			size_t		position_embed_size;
			size_t		lm_head_size;
			size_t		embed_table_count;
			size_t		position_embed_count;
			size_t		lm_head_count;
			...
			embed_table_count = embed_table_size / sizeof(float);
			position_embed_count = position_embed_size / sizeof(float);
			lm_head_count = lm_head_size / sizeof(float);

			NBP_ALLOC(entry->weights.embedding_table, float, embed_table_count);
			...
			NBP_ALLOC(entry->weights.position_embeddings, float, position_embed_count);
			...
			NBP_ALLOC(entry->weights.lm_head_weights, float, lm_head_count);
							 ...
							 char **errstr)
{
	...
	while (*p && *p != '}')
	{
		...
		*p = '\0';				/* Null-terminate key */
		...
		{
			/*
			 * Parse logit bias map: {"token_id": bias_value, ...} or
			 * [token_id, bias_value, ...]
			 */
			if (*p == '{')
			{
					...
					*p = '\0';	/* Null-terminate value */
					...
					PG_TRY();
					{
							...
							<= NDB_HF_MAX_STOP_SEQ_LEN)
						{
							...
							/*
							 * Skip CLS token (first token) and SEP token
							...
							*p = '\0';	/* Null-terminate value */
							...
							PG_TRY();
							{
									...
									<= NDB_HF_MAX_STOP_SEQ_LEN)
								{
									...
									/*
									 * Skip CLS token (first token) and SEP
									 * token (last token) if present
									 */
									int			start_idx =
										(stop_token_len > 0
										 && stop_token_ids
								...
								else
								{
									...
									{
										/*
										 * Use simplified tokenization for
										 * fallback
										 */
										gen_params
											->stop_seq_lens
							...
							/*
							 * ONNX runtime not available, use word count
							 * fallback
							 */
							const char *ptr;
							int			word_count;
						  ...
						  char **errstr)
{
	...
	cuda_status = cudaMalloc((void **) &kv_cache->value_cache, cache_bytes);
	if (cuda_status != cudaSuccess)
	{
...
/*
 * ndb_cuda_hf_complete
 *	  Generate text completion using CUDA-accelerated Hugging Face model.
 *
					 ...
					 char **errstr)
{
	...
	if (params_json && strlen(params_json) > 0)
	{
		...
		/* Map from NdbGenParams to NdbCudaHfGenParams */
	...
	/* Initialize KV cache */
	memset(&kv_cache, 0, sizeof(NdbCudaHfKVCache));
	{
		char	   *temp_errstr = NULL;
							...
							char **errstr)
{
	...
	/* Find or load model (reuse existing logic from ndb_cuda_hf_complete) */
	entry = ndb_cuda_hf_find_model(model_name);
	if (entry == NULL || !entry->loaded)
	{
		/* Load model */
	...
	/* Initialize KV cache */
	memset(&kv_cache, 0, sizeof(NdbCudaHfKVCache));
	rc = ndb_cuda_hf_init_kv_cache(&kv_cache, &config, errstr);
	if (rc != 0)
	{
		MemoryContextSwitchTo(oldcontext);
		MemoryContextDelete(stream_context);
	...
	/*
	 * NOTE: Current implementation generates all tokens first, then calls
	 * callback for each. For true streaming (token-by-token during
	 * generation), the CUDA kernel ndb_cuda_hf_generate_inference would need
	 * to be modified to support incremental generation with callbacks. This
	 * requires kernel-level changes to support pausing generation after each
	 * token and calling the callback. For now, we generate all tokens and
	 * then stream them via callback.
	 */
	rc = ndb_cuda_hf_generate_inference(model_name,
										input_token_ids,
										input_seq_len,
...
/*
 * ndb_cuda_hf_generate_batch
 *	  Generate text for multiple prompts in batch
 *
 * This function processes multiple prompts in parallel using CUDA streams,
						   ...
						   char **errstr)
{
	...
	{
		memset(&results[i], 0, sizeof(NdbCudaHfBatchResult));
		results[i].status = -1;
	}
 ...
 *
 * Full CUDA kernel implementation for reranking with cross-encoder architecture.
				   ...
				   char **errstr)
{
	...
	/*
	 * CUDA HF reranking implementation framework:
	 *
	 * Full implementation requires: 1. Load cross-encoder model (BERT-based
	 * reranker) if not cached 2. Tokenize query + each document (concatenate
	 * for each pair) 3. Run cross-encoder on GPU (batch process
	 * query-document pairs) 4. Extract relevance scores from model output
	 * logits 5. Return scores in descending order
	 */
	...
	if (entry == NULL || !entry->loaded)
	{
		/* Load model - would use ndb_cuda_hf_load_model() */
		/* For now, fall back to HTTP/ONNX */
		if (errstr)
			*errstr = pstrdup("CUDA HF reranking model not loaded - use "
	...
	if (!entry->weights_on_device)
	{
		...
		cuda_err = cudaMalloc(&entry->device_weights_ptr, weights_size);
		if (cuda_err != cudaSuccess)
		{
			if (errstr)
		...
		cuda_err = cudaMemcpy(entry->device_weights_ptr,
							  entry->weights.embedding_table,
							  weights_size,
							  cudaMemcpyHostToDevice);
		if (cuda_err != cudaSuccess)
		{
			cudaFree(entry->device_weights_ptr);
...
/*
 * ndb_cuda_hf_unload_model
 *	  Unload a Hugging Face model from GPU memory
 */
int
ndb_cuda_hf_unload_model(const char *model_name, char **errstr)
{
	NdbCudaHfModelEntry *entry = NULL;
	NdbCudaHfModelEntry *prev = NULL;
	cudaError_t cuda_status;
...
/*
 * ndb_cuda_hf_model_loaded
 *	  Check if a model is loaded in cache
 */
bool
ndb_cuda_hf_model_loaded(const char *model_name)
{
	NdbCudaHfModelEntry *entry;
