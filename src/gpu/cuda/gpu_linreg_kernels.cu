 ...
 */
__global__ void
ndb_cuda_build_X_matrix_kernel(const float *features,
	...
	int dim_with_intercept)
{
	...
	ndb_cuda_build_X_matrix_kernel<<<build_blocks, build_threads>>>(
		features,
		X_with_intercept,
		n_samples,
		feature_dim,
 ...
 */
__global__ void
ndb_cuda_linreg_eval_kernel(const float *features,
	const double *targets,
	...
	long long *count_out)
{
	...
	/* Launch kernel */
	ndb_cuda_linreg_eval_kernel<<<blocks, threads_per_block>>>(
		features,
		targets,
		coefficients,
		intercept,
