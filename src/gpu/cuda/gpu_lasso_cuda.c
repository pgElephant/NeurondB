		     ...
		     char **errstr)
{
	...
	cuda_err = cudaMemcpy(d_residuals, residuals, residual_bytes, cudaMemcpyHostToDevice);
	if (cuda_err != cudaSuccess)
	{
		if (errstr)
	...
	for (iter = 0; iter < max_iters && !converged; iter++)
	{
		...
		for (j = 0; j < feature_dim; j++)
		{
			...
			cuda_err = cudaMemcpy(h_rho, d_rho, sizeof(double), cudaMemcpyDeviceToHost);
			if (cuda_err != cudaSuccess)
			{
				if (errstr)
			...
			cuda_err = cudaMemcpy(h_z, d_z, sizeof(double), cudaMemcpyDeviceToHost);
			if (cuda_err != cudaSuccess)
			{
				if (errstr)
	...
	cuda_err = cudaMemcpy(residuals, d_residuals, residual_bytes, cudaMemcpyDeviceToHost);
	if (cuda_err != cudaSuccess)
	{
		if (errstr)
	...
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("neurondb: ndb_cuda_lasso_evaluate: model_data is NULL");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("neurondb: ndb_cuda_lasso_evaluate: features is NULL");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("neurondb: ndb_cuda_lasso_evaluate: targets is NULL");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("neurondb: ndb_cuda_lasso_evaluate: output pointers are NULL");
		return -1;
	}
	...
	/* Convert coefficients from float to double and copy to GPU */
	{
		...
		if (h_coefficients_double == NULL)
		{
			...
			cudaFree(d_count);
			if (errstr)
				*errstr = pstrdup("neurondb: ndb_cuda_lasso_evaluate: failed to allocate host memory for coefficients");
			return -1;
		}
