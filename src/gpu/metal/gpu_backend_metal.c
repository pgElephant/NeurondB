		   ...
		   char **errstr)
{
	...
	elog(DEBUG1, "ndb_metal_nb_train: starting training: n_samples=%d, feature_dim=%d, class_count=%d",
		    ...
		    char **errstr)
{
	...
	elog(DEBUG1, "ndb_metal_gmm_train: starting training: n_samples=%d, feature_dim=%d, n_components=%d",
	...
	for (iter = 0; iter < max_iters; iter++)
	{
		...
		{
			elog(DEBUG1, "ndb_metal_gmm_train: converged at iteration %d (ll=%.4f)", iter + 1, log_likelihood);
			break;
		}
	...
	char **errstr)
{
	...
	elog(DEBUG1, "ndb_metal_knn_train: starting training: n_samples=%d, feature_dim=%d, k=%d, task_type=%d",
