...
neurondb_gpu_rocm_init(void)
{
	...
	{
		elog(WARNING, "neurondb: No ROCm/HIP devices found: %s",
	...
	{
		elog(WARNING, "neurondb: Failed to set HIP device %d: %s",
			 rocm_device_id, hipGetErrorString(hip_err));
		return false;
	}
	...
	if (hip_err == hipSuccess)
	{
		...
		elog(LOG, "neurondb: ✅ Compute units: %d",
	...
	{
		elog(WARNING, "neurondb: Failed to create rocBLAS handle: %d",
			 rocblas_status);
		return false;
	}
	...
	if (hip_err != hipSuccess)
	{
		...
		elog(WARNING, "neurondb: Failed to create HIP stream: %s",
			 hipGetErrorString(hip_err));
		return false;
	}
...
neurondb_gpu_rocm_cleanup(void)
{
	...
	if (total_rocm_ops > 0)
		elog(LOG, "neurondb: ROCm cleanup: %llu GPU operations performed",
...
neurondb_gpu_rocm_l2_distance(const float *a, const float *b, int dim)
{
	...
	if (hip_err != hipSuccess) return -1.0f;
 ...
 */
void
neurondb_gpu_rocm_batch_l2(const float *queries, const float *targets,
							int num_queries, int num_targets, int dim,
							float *distances)
{
	/* Implementation uses HIP kernel for parallel computation */
...
/* Statistics */
uint64_t neurondb_gpu_rocm_get_operations_count(void) { return total_rocm_ops; }

#else /* !NDB_GPU_HIP */

/* Stub functions when ROCm is not available */
bool neurondb_gpu_rocm_init(void) { return false; }
void neurondb_gpu_rocm_cleanup(void) { }
bool neurondb_gpu_rocm_is_available(void) { return false; }
const char *neurondb_gpu_rocm_device_name(void) { return "Not compiled"; }
float neurondb_gpu_rocm_l2_distance(const float *a, const float *b, int dim) { (void)a; (void)b; (void)dim; return -1.0f; }
float neurondb_gpu_rocm_cosine_distance(const float *a, const float *b, int dim) { (void)a; (void)b; (void)dim; return -1.0f; }
float neurondb_gpu_rocm_inner_product(const float *a, const float *b, int dim) { (void)a; (void)b; (void)dim; return -1.0f; }
void neurondb_gpu_rocm_batch_l2(const float *q, const float *t, int nq, int nt, int d, float *dist) { (void)q; (void)t; (void)nq; (void)nt; (void)d; (void)dist; }
uint64_t neurondb_gpu_rocm_get_operations_count(void) { return 0; }
