                    ...
                    int k, int max_iters, float *centroids, int *assignments)
{
    ...
    {
        elog(DEBUG1, "neurondb: active GPU backend lacks k-means support; falling back to CPU");
        return;
    }

    for (iter = 0; iter < max_iters; iter++)
    {
        if (backend->launch_kmeans_assign(vectors, centroids, assignments,
                                          num_vectors, dim, k, NULL) != 0)
        {
            elog(DEBUG1, "neurondb: GPU k-means assign failed (iter %d); falling back", iter);
            return;
        }

        if (backend->launch_kmeans_update(vectors, assignments, centroids,
                                          num_vectors, dim, k, NULL) != 0)
        {
            elog(DEBUG1, "neurondb: GPU k-means update failed (iter %d); falling back", iter);
            return;
        }
    }
}
                    ...
                    float eps, int min_pts, int *labels)
{
    ...
    elog(DEBUG1, "neurondb: GPU DBSCAN not implemented; using CPU fallback");
}
