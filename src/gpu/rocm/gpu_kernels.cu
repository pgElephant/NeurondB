 ...
 *
 * gpu_kernels.cu
 *    HIP kernels for NeurondB GPU acceleration on AMD GPUs
 *
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_fp16_hip(const float *input,
	void *output,
	int count,
	hipStream_t stream)
{
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_int8_hip(const float *input,
	signed char *output,
	int count,
	float scale,
	hipStream_t stream)
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_binary_hip(const float *input,
	unsigned char *output,
	int count,
	hipStream_t stream)
{
	int block = 256;
	int grid = ((count + 7) / 8 + block - 1) / block;
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_int4_hip(const float *input,
	unsigned char *output,
	int count,
	float scale,
	hipStream_t stream)
{
	int block = 256;
	int grid = ((count + 1) / 2 + block - 1) / block;
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_fp8_e4m3_hip(const float *input,
	unsigned char *output,
	int count,
	hipStream_t stream)
{
 ...
 */
extern "C" hipError_t
launch_quantize_fp32_to_fp8_e5m2_hip(const float *input,
	unsigned char *output,
	int count,
	hipStream_t stream)
{
