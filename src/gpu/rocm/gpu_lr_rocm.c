				  ...
				  char **errstr)
{
	...
	/* Convert features from row-major to column-major for rocBLAS compatibility */
	/* rocBLAS expects column-major layout: X[i + j * n_samples] = features[i * feature_dim + j] */
	...
	for (iter = 0; iter < max_iters; iter++)
	{
		...
		/* Forward pass: compute z = X * w + b using rocBLAS SGEMV (optimized) */
		{
			...
			/* Defensive: Validate alpha and beta are finite before rocBLAS call */
			if (!isfinite(alpha))
			{
				elog(WARNING,
		...
		/* grad_bias = sum(predictions - labels) / n_samples */
		{
			...
			/* Use rocBLAS SGEMV with column-major features (converted before training loop) */
			if (handle != NULL)
			{
			...
			/* Fallback to kernel-based gradient computation if rocBLAS failed or handle is NULL */
