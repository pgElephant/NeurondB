 ...
 *
 * gpu_nb_cuda.c
 *    ROCm backend bridge for Naive Bayes training and prediction.
 *
...
int
ndb_rocm_nb_pack_model(const GaussianNBModel *model,
	bytea **model_data,
	Jsonb **metrics,
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("invalid NB model for HIP pack: model or model_data is NULL");
		return -1;
	}
...
int
ndb_rocm_nb_train(const float *features,
	const double *labels,
	int n_samples,
	int feature_dim,
	int class_count,
	...
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: features array is NULL");
		return -1;
	}
	if (labels == NULL)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: labels array is NULL");
		return -1;
	}
	if (model_data == NULL)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: model_data output pointer is NULL");
		return -1;
	}
	if (n_samples <= 0 || n_samples > 100000000)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: n_samples must be between 1 and 100000000");
		return -1;
	}
	if (feature_dim <= 0 || feature_dim > 1000000)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: feature_dim must be between 1 and 1000000");
		return -1;
	}
	if (class_count <= 0 || class_count > 1000000)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: class_count must be between 1 and 1000000");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: feature array size exceeds MaxAllocSize");
		return -1;
	}
	...
	for (i = 0; i < n_samples; i++)
	{
		...
		{
			if (errstr)
				*errstr = pstrdup("HIP NB train: non-finite value in labels array");
			goto cleanup;
		}
		...
		{
			if (errstr)
				*errstr = pstrdup("HIP NB train: invalid class label in labels array (must be integral 0 or 1 for binary)");
			goto cleanup;
		}
	}
	...
	for (i = 0; i < n_samples; i++)
	{
		for (j = 0; j < feature_dim; j++)
		{
			...
			{
				if (errstr)
					*errstr = pstrdup("HIP NB train: non-finite value in features array");
				goto cleanup;
			}
		}
	}

	/* Step 1: Count samples per class using HIP
	 * NOTE: This may fail due to HIP context issues in forked PostgreSQL backends.
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB train: n_samples is zero (division by zero)");
		goto cleanup;
	}
	for (i = 0; i < class_count; i++)
	{
		if (class_counts[i] > 0)
		{
			...
			{
				if (errstr)
					*errstr = pstrdup("HIP NB train: computed invalid class prior");
				goto cleanup;
			}
		}
		else
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP mean computation failed");
		goto cleanup;
	}

	/* Step 4: Compute variances using HIP */
	if (ndb_rocm_nb_compute_variances(features, labels, means, n_samples, feature_dim, class_count, variances, class_counts) != 0)
	{
		if (errstr)
			*errstr = pstrdup("HIP variance computation failed");
		goto cleanup;
	}
	...
	if (ndb_rocm_nb_pack_model(&model, &blob, metrics ? &metrics_json : NULL, errstr) != 0)
	{
		if (errstr && *errstr == NULL)
			*errstr = pstrdup("HIP NB model packing failed");
		goto cleanup;
	}
...
int
ndb_rocm_nb_predict(const bytea *model_data,
	const float *input,
	int feature_dim,
	int *class_out,
	...
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("invalid parameters for HIP NB prediction");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB predict: model_data too small (corrupted or invalid)");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB predict: invalid n_classes in model header");
		return -1;
	}
	if (hdr->n_features <= 0 || hdr->n_features > 1000000)
	{
		if (errstr)
			*errstr = pstrdup("HIP NB predict: invalid n_features in model header");
		return -1;
	}
	if (feature_dim != hdr->n_features)
	{
		if (errstr)
			*errstr = psprintf("HIP NB predict: feature dimension mismatch (expected %d, got %d)", hdr->n_features, feature_dim);
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB predict: model_data size mismatch (corrupted model)");
		return -1;
	}
	...
	for (i = 0; i < feature_dim; i++)
	{
		...
		{
			if (errstr)
				*errstr = pstrdup("HIP NB predict: non-finite value in input array");
			return -1;
		}
	}
 ...
 */
int
ndb_rocm_nb_predict_batch(const bytea *model_data,
	...
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("invalid inputs for HIP NB batch predict");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = pstrdup("HIP NB batch predict: model_data too small");
		return -1;
	}
	...
	{
		if (errstr)
			*errstr = psprintf("HIP NB batch predict: feature dimension mismatch (expected %d, got %d)",
	...
	for (i = 0; i < n_samples; i++)
	{
		...
		rc = ndb_rocm_nb_predict(model_data,
			input,
			feature_dim,
			&class_out,
			&probability_out,
 ...
 */
int
ndb_rocm_nb_evaluate_batch(const bytea *model_data,
	const float *features,
	const int *labels,
	int n_samples,
	int feature_dim,
	...
	char **errstr)
{
	...
	{
		if (errstr)
			*errstr = pstrdup("invalid inputs for HIP NB batch evaluate");
		return -1;
	}
...
/* Stubs for non-HIP builds - match function signatures from header */
	...
	char **errstr)
{
	...
	(void) metrics;
	if (errstr)
		*errstr = pstrdup("HIP Naive Bayes not built with GPU support");
	return -1;
}
...
int
ndb_rocm_nb_pack_model(const struct GaussianNBModel *model,
	bytea **model_data,
	Jsonb **metrics,
	char **errstr)
{
