...
prometheus_register_worker(void)
{
    ...
    elog(LOG, "neurondb: Registered Prometheus exporter worker on port %d",
         prometheus_port);
}
...
prometheus_shmem_init(void)
{
    ...
    prom_metrics = (PrometheusMetrics *)
        ShmemInitStruct("neurondb_prometheus_metrics",
                        prometheus_shmem_size(), &found);

    if (!found)
    {
        /* Single LWLock per metrics struct, grabbed from our named tranche */
        prom_metrics->lock = &(GetNamedLWLockTranche("neurondb_prometheus"))->lock;
...
prometheus_worker_main(Datum arg)
{
    ...
    elog(LOG, "neurondb: Prometheus exporter starting on %s:%d",
    ...
    {
        ereport(ERROR,
                (errmsg("neurondb: Failed to create Prometheus socket: %m")));
    }
    ...
    if (setsockopt(listen_socket, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0)
    {
        close(listen_socket);
        ereport(ERROR,
                (errmsg("neurondb: setsockopt(SO_REUSEADDR) failed: %m")));
    }
    ...
    if (bind(listen_socket, (struct sockaddr *)&server_addr, sizeof(server_addr)) < 0)
    {
        close(listen_socket);
        ereport(ERROR,
                (errmsg("neurondb: Failed to bind Prometheus socket to %s:%d: %m",
                        prometheus_host, prometheus_port)));
    }
    ...
    if (listen(listen_socket, 5) < 0)
    {
        close(listen_socket);
        ereport(ERROR,
                (errmsg("neurondb: Failed to listen on Prometheus socket: %m")));
    }
...
send_metrics(int socket)
{
    ...
    appendStringInfo(&metrics,
        "# HELP %s Total number of queries (all types, successes and errors)\n"
        "# TYPE %s counter\n"
        "%s %lld\n\n",
        METRIC_QUERIES_TOTAL, METRIC_QUERIES_TOTAL, METRIC_QUERIES_TOTAL,
    ...
    appendStringInfo(&metrics,
        ...
        "%s_sum %.6f\n"
        "%s_max %.6f\n\n",
        METRIC_QUERY_DURATION, METRIC_QUERY_DURATION,
        METRIC_QUERY_DURATION, prom_metrics->query_duration_sum,
        METRIC_QUERY_DURATION, prom_metrics->query_duration_max);
    ...
    appendStringInfo(&metrics,
        ...
        "# TYPE %s gauge\n"
        "%s %lld\n\n",
        METRIC_VECTOR_COUNT, METRIC_VECTOR_COUNT, METRIC_VECTOR_COUNT,
    ...
    appendStringInfo(&metrics,
        ...
        "# TYPE %s gauge\n"
        "%s %lld\n\n",
        METRIC_INDEX_SIZE, METRIC_INDEX_SIZE, METRIC_INDEX_SIZE,
    ...
    appendStringInfo(&metrics,
        ...
        "# TYPE %s counter\n"
        "%s %lld\n\n",
        METRIC_CACHE_HITS, METRIC_CACHE_HITS, METRIC_CACHE_HITS,
    ...
    appendStringInfo(&metrics,
        ...
        "# TYPE %s counter\n"
        "%s %lld\n\n",
        METRIC_CACHE_MISSES, METRIC_CACHE_MISSES, METRIC_CACHE_MISSES,
    ...
    appendStringInfo(&metrics,
        ...
        "%s{status=\"active\"} %d\n"
        "%s{status=\"idle\"} %d\n\n",
        METRIC_WORKER_STATUS, METRIC_WORKER_STATUS,
        METRIC_WORKER_STATUS, prom_metrics->workers_active,
        METRIC_WORKER_STATUS, prom_metrics->workers_idle);
...
neurondb_prometheus_metrics(PG_FUNCTION_ARGS)
{
    ...
    if (prom_metrics == NULL)
        ereport(ERROR,
                (errmsg("neurondb: Prometheus metrics not initialized")));
