...
distributed_knn_search(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		if (nshards == 0)
			ereport(ERROR, (errmsg("no shards specified for distributed kNN search")));
		...
		elog(NOTICE, "neurondb: distributed kNN search across %d shards (k=%d)", nshards, k);
		...
		/* Collect candidates from each shard using SPI */
		{
			...
			for (i = 0; i < nshards; i++)
			{
				...
				if (ret != SPI_OK_CONNECT)
					elog(ERROR, "neurondb: SPI_connect failed for shard \"%s\": code %d", shard_names[i], ret);

				ret = SPI_execute(sql.data, true, 0);
				if (ret != SPI_OK_SELECT)
				{
					SPI_finish();
					elog(ERROR, "neurondb: SPI SELECT failed on shard \"%s\": %s", shard_names[i], sql.data);
				}
			...
			/* Global stable sort and SRF context build */
			{
				...
				int *sorted_idxs = (int *) palloc(sizeof(int) * result_count);
				...
				{
					int		min_idx = i;
					float4	min_dist = DatumGetFloat4(candidate_dists[sorted_idxs[i]]);
					int		j;
					for (j = i + 1; j < result_count; j++)
					{
						float4 dist_j = DatumGetFloat4(candidate_dists[sorted_idxs[j]]);
						if (dist_j < min_dist)
						{
							min_dist = dist_j;
							min_idx = j;
					...
					if (min_idx != i)
					{
						...
						sorted_idxs[i] = sorted_idxs[min_idx];
...
merge_distributed_results(PG_FUNCTION_ARGS)
{
	...
	deconstruct_array(shard_results, ANYARRAYOID, -1, false, 'd',
					  &subarrays, &nulls, &nelems);
	num_shards = nelems;
...
#define NREPLICAS 3
	static const char *replicas[NREPLICAS] = {"replica-1", "replica-2", "replica-3"};
	...
	elog(DEBUG1, "neurondb: starting replica selection for query_type=%s (k=%d)", type_str, k);

	for (i = 0; i < NREPLICAS; i++)
	{
		scores[i] = latencies[i] * (1.0f - recalls[i]);
		elog(DEBUG1, "neurondb: replica %s latency=%.2f recall=%.2f score=%.5f",
	...
	elog(DEBUG1, "neurondb: selected replica: %s (score=%.5f)", replicas[best], scores[best]);
...
sync_index_async(PG_FUNCTION_ARGS)
{
	...
	elog(NOTICE, "neurondb: initiating async WAL streaming (index \"%s\" -> replica \"%s\")",
		 idx_str, replica_str);
	...
	resetStringInfo(&sql);
	appendStringInfo(&sql, "SELECT 1 FROM pg_replication_slots WHERE slot_name = '%s'",
					 slot_name.data);
	...
	if (!slot_exists)
	{
		elog(NOTICE, "neurondb: creating logical replication slot \"%s\"", slot_name.data);
		resetStringInfo(&sql);
		appendStringInfo(&sql, "SELECT pg_create_logical_replication_slot('%s','pgoutput')",
						 slot_name.data);
		ret = SPI_execute(sql.data, false, 0);
		if (ret != SPI_OK_SELECT)
		{
			...
			ereport(ERROR,
					(errcode(ERRCODE_INTERNAL_ERROR),
					 errmsg("neurondb: failed to create replication slot \"%s\"", slot_name.data)));
		}
	...
	{
		elog(NOTICE, "neurondb: creating publication \"%s\" for table %s",
			 pub_name.data, idx_str);
		resetStringInfo(&sql);
		appendStringInfo(&sql, "CREATE PUBLICATION %s FOR TABLE %s", pub_name.data, idx_str);
		ret = SPI_execute(sql.data, false, 0);
		if (ret != SPI_OK_UTILITY)
			elog(WARNING, "neurondb: failed to create publication \"%s\" (may require manual intervention)",
				 pub_name.data);
		else
	...
	if (plan == NULL)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("neurondb: failed to prepare sync metadata INSERT/UPDATE")));
	}
	...
	if (ret != SPI_OK_INSERT && ret != SPI_OK_UPDATE)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
				 errmsg("neurondb: failed to insert/update sync metadata")));
	}
	SPI_freeplan(plan);
