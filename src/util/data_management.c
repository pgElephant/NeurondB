...
vector_time_travel(PG_FUNCTION_ARGS)
{
	 ...
	 * Use temporal columns 'valid_from' and 'valid_to' for bitemporal versioning
	 * Return the total count of valid rows as of the given system_time
	 */
	initStringInfo(&sql);
...
vacuum_vectors(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Gather statistics: number of vectors and average dimension over vectors.
	 * This assumes each row has a 'embedding' column that can use vector_dims()
	 */
	appendStringInfo(&sql,
		"WITH stats AS ("
		"    SELECT COUNT(*) AS n, "
...
rebalance_index(PG_FUNCTION_ARGS)
{
	 ...
	 *   - examine all node-layers of HNSW topology,
	 *   - redistribute neighbors so each node's layer is balanced,
	 *   - ensure edge constraints (M and M_max) are satisfied,
