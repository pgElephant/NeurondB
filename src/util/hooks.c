...
register_custom_operator(PG_FUNCTION_ARGS)
{
	...
	elog(NOTICE, "neurondb: registering custom operator '%s' -> '%s'",
		 name_str, func_str);
...
assert_recall(PG_FUNCTION_ARGS)
{
	...
	if (passed)
		elog(NOTICE, "neurondb: TEST PASSED: recall=%.4f (expected=%.4f ±%.4f)",
			 actual_recall, expected_recall, tolerance);
	else
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("neurondb: TEST FAILED: recall=%.4f (expected=%.4f ±%.4f)",
						actual_recall, expected_recall, tolerance)));
...
assert_vector_equal(PG_FUNCTION_ARGS)
{
	...
	if (vec1->dim != vec2->dim)
	{
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("neurondb: TEST FAILED: dimension mismatch %d != %d",
						vec1->dim, vec2->dim)));
	}
	...
	if (passed)
		elog(NOTICE, "neurondb: TEST PASSED: vectors equal within tolerance %.6f",
			 tolerance);
	else
		ereport(ERROR,
				(errcode(ERRCODE_DATA_EXCEPTION),
				 errmsg("neurondb: TEST FAILED: vectors differ at position %d", i)));
