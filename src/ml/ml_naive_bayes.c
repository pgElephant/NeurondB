			 ...
			 int *out_dim)
{
	...
	/* Extract data - track valid_rows to avoid uninitialized memory */
	for (i = 0; i < nvec; i++)
	{
		...
		/* Validate label is exactly 0 or 1 - label is already cast to float8 in query */
		{
		...
		if (feat_is_array)
		{
			...
			/* Copy array data to float array */
	...
	/* Compute class priors - handle empty classes correctly */
	{
...
nb_evaluate_naive_bayes_internal(FunctionCallInfo fcinfo)
{
 ...
 cpu_evaluation_path:
	 ...
	 /* BULLETPROOF: Build JSONB result in SPI context */
	 ...
	 char **errstr)
 {
	 ...
	 if (backend->init && backend->init() != 0)
	 {
		 if (errstr)
			 *errstr =
				 pstrdup("Failed to initialize GPU backend");
		 return false;
	 }
