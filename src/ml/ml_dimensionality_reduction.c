...
reduce_tsne(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		/* Compute pairwise squared distances */
		{
			...
			/* Use binary search to find sigma for each point to achieve target perplexity */
				...
				for (i = 0; i < nvec; i++)
				{
					...
					for (j = 0; j < nvec; j++)
					{
						if (i != j)
						{
							...
							Q[i][j] = 1.0 / (1.0 + dist_sq); /* t-distribution with df=1 */
							sum_q += Q[i][j];
						}
					}
...
reduce_umap(PG_FUNCTION_ARGS)
{
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		/* UMAP algorithm implementation */
		{
			...
			{
				double sigma = 0.0;
				double rho = distances[i][neighbors[i][0]]; /* Distance to nearest neighbor */

				/* Compute sigma using binary search to achieve target perplexity */
				{
