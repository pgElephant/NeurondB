...
static bool rf_select_split(const float *features, const double *labels,
			    const int *indices, int count, int feature_dim,
			    int n_classes, pg_prng_state *rng,
			    int *feature_order, int *best_feature,
			    double *best_threshold, double *best_impurity);
static int
rf_build_branch_tree(GTree *tree, const float *features, const double *labels,
		     const double *feature_vars, int feature_dim, int n_classes,
		     const int *indices, int count, int depth, int max_depth,
			    ...
			    double *best_threshold, double *best_impurity)
{
	...
	for (f = 0; f < candidates; f++) {
		...
		for (i = 0; i < pair_count - 1; i++) {
			...
			left_imp = rf_gini_from_counts(left_counts_tmp,
						       n_classes, left_total);
			right_imp = rf_gini_from_counts(right_counts_tmp,
							n_classes, right_total);
			threshold_candidate =
		     ...
		     double *feature_importance, double *max_split_deviation)
{
...

	{
		...
		if (left_count == 0 || right_count == 0) {
			...
			node_idx = gtree_add_split(tree, split_feature,
						   split_threshold);

			left_child = rf_build_branch_tree(
				tree, features, labels, feature_vars,
				feature_dim, n_classes, left_indices,
				left_count, depth + 1, max_depth, min_samples,
				rng, feature_order, feature_importance,
				max_split_deviation);
			right_child = rf_build_branch_tree(
				tree, features, labels, feature_vars,
				feature_dim, n_classes, right_indices,
				right_count, depth + 1, max_depth, min_samples,
				rng, feature_order, feature_importance,
				max_split_deviation);
...
static void
rf_store_model(int32 model_id, int n_features, int n_samples, int n_classes,
	       double majority, double fraction, double gini, double entropy,
	       const int *class_counts, const double *feature_means,
	       const double *feature_variances,
	       const double *feature_importance, GTree *tree, int split_feature,
	       double split_threshold, double second_value,
	       double second_fraction, double left_value, double left_fraction,
	       double right_value, double right_fraction, double max_deviation,
	       double max_split_deviation, int feature_limit,
	       const double *left_means, const double *right_means,
	       int tree_count, GTree *const *trees, const double *tree_majority,
	       const double *tree_majority_fraction, const double *tree_second,
	       const double *tree_second_fraction,
	       const double *tree_oob_accuracy, double oob_accuracy)
{
...
Datum train_random_forest_classifier(PG_FUNCTION_ARGS)
{
	...
	if (PG_NARGS() > 3 && !PG_ARGISNULL(3)) {
		...
		if (arg_trees < 1)
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("random_forest: number of trees must "
					"be at least 1")));
		if (arg_trees > 1024)
			arg_trees = 1024;
	...
	if (PG_NARGS() > 4 && !PG_ARGISNULL(4)) {
		...
		if (arg_depth < 1)
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("random_forest: max depth must be at "
					"least 1")));
	...
	if (PG_NARGS() > 5 && !PG_ARGISNULL(5)) {
		...
		if (arg_min_samples < 1)
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("random_forest: min samples must be at "
					"least 1")));
		...
		if (feature_dim > 0 && sample_count > 0 && n_classes > 0 &&
			...
			if (best_split_valid) {
				...
				elog(DEBUG1,
				     ...
				     "feature=%d threshold=%.3f impurity=%.6f",
		...
		if (feature_dim > 0 && feature_means_tmp != NULL &&
			...
			for (i = 0; i < n_classes; i++) {
				if (left_total > 0 &&
				    (left_majority_idx < 0 ||
				     left_counts[i] >
					     left_counts[left_majority_idx]))
					left_majority_idx = i;
				if (right_total > 0 &&
				    (right_majority_idx < 0 ||
				     right_counts[i] >
					     right_counts[right_majority_idx]))
			...
			if (feature_limit > 0 &&
				...
				for (f = 0; f < feature_limit; f++) {
					if (left_feature_counts_vec != NULL &&
					    left_feature_counts_vec[f] > 0)
						left_branch_means_vec[f] =
							left_feature_sums_vec
								[f] /
							(double)left_feature_counts_vec
								[f];
					...
					if (right_feature_counts_vec != NULL &&
					    right_feature_counts_vec[f] > 0)
						right_branch_means_vec[f] =
							right_feature_sums_vec
								[f] /
							(double)right_feature_counts_vec
								[f];
					else if (feature_means_tmp != NULL &&
						 f < feature_dim)
						right_branch_means_vec[f] =
	...
	if (majority_count > 0) {
		...
		if (forest_trees > 0) {
			trees = (GTree **)palloc0(sizeof(GTree *) *
						  forest_trees);
			tree_majorities = (double *)palloc0(sizeof(double) *
							    forest_trees);
			tree_majority_fractions = (double *)palloc0(
				sizeof(double) * forest_trees);
			tree_seconds = (double *)palloc0(sizeof(double) *
							 forest_trees);
			tree_second_fractions = (double *)palloc0(
				sizeof(double) * forest_trees);
			tree_oob_accuracy = (double *)palloc0(sizeof(double) *
							      forest_trees);
		}
		...
		for (t = 0; t < forest_trees; t++) {
			...
			if (n_samples > 0)
				inbag = (bool *)palloc0(sizeof(bool) *
							n_samples);

			if (sample_target > 0 && n_samples > 0)
				tree_bootstrap = (int *)palloc(sizeof(int) *
							       sample_target);
			...
			int feature_for_split = tree_feature;
			...
			if (feature_dim > 0 && feature_means_tmp != NULL) {
				...
				node_idx =
					gtree_add_split(tree, feature_for_split,
							tree_threshold);

				if (left_indices_local != NULL &&
				    left_index_count > 0)
					left_idx = rf_build_branch_tree(
						tree, stage_features, labels,
				...
				if (right_indices_local != NULL &&
				    right_index_count > 0)
					right_idx = rf_build_branch_tree(
						tree, stage_features, labels,
						feature_vars_tmp, feature_dim,
				...
				gtree_set_child(tree, node_idx, right_idx,
						false);
				...
				if (tree_count == 0) {
					...
					right_branch_fraction =
						tree_right_fraction;
					branch_threshold = tree_threshold;
					branch_threshold_valid = true;
				...
				elog(DEBUG1,
				     "random_forest: tree %d split feature=%d "
				     "threshold=%.3f left=%.3f right=%.3f",
				     t + 1, feature_for_split, tree_threshold,
				     tree_left_value, tree_right_value);
			} else {
				left_idx =
	...
	rf_store_model(
		model_id, feature_dim, n_samples, n_classes, majority_value,
		majority_fraction, gini_impurity, label_entropy,
...
static double rf_tree_predict_single(const GTree *tree, const RFModel *model,
				     const Vector *vec, double *left_dist,
				     double *right_dist, int *leaf_out)
{
	const GTreeNode *nodes;
	int idx;
	int steps = 0;
	...
	while (idx >= 0 && idx < tree->count) {
		...
		if (vec == NULL || node->feature_idx < 0 ||
			...
			elog(DEBUG1,
			     "random_forest: path aborted at node %d (feature "
			     "%d)",
...
Datum predict_random_forest(PG_FUNCTION_ARGS)
{
	...
	if (!PG_ARGISNULL(1))
		...
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			 errmsg("random_forest: feature vector is required")));
	...
	if (model->n_features > 0 && feature_vec != NULL &&
		...
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			 errmsg("random_forest: feature dimension mismatch "
				"(expected %d got %d)",
	...
	if (model->feature_means != NULL && feature_vec != NULL) {
		...
		if (model->feature_variances != NULL &&
			...
			elog(DEBUG1,
			     "random_forest: deviation %.3f exceeds threshold, "
	...
	if (model->tree_count > 0 && model->trees != NULL) {
		for (i = 0; i < model->tree_count; i++) {
			...
			tree_result = rf_tree_predict_single(
				tree, model, feature_vec, &tree_left,
				&tree_right, &leaf_idx);
	...
	if (model->feature_variances != NULL && fallback_fraction > 0.0 &&
		...
		elog(DEBUG1,
		     "random_forest: high deviation %.3f -> returning second "
		     "class %.3f",
	...
	elog(DEBUG1,
	     "predict_random_forest: returning %.3f (branch %s leaf %.3f frac "
...
Datum evaluate_random_forest(PG_FUNCTION_ARGS)
{
	...
	elog(NOTICE,
	     ...
	     "topimp=%.3f",
	     model->n_samples, model->n_classes, accuracy, model->gini_impurity,
	     model->label_entropy, model->oob_accuracy,
	     model->majority_fraction, top_feature_idx, top_feature_importance);
	...
	result_array = construct_array(result_datums, 4, FLOAT8OID,
