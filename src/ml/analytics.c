 ...
 * - min_pts: Minimum points required to form dense region
 * - Returns cluster IDs (-1 = noise/outlier)
 */
...
typedef struct DBSCANState
{
	...
	double		eps;			/* Epsilon radius */
	int			min_pts;		/* Minimum points */
	int			next_cluster;	/* Next cluster ID */
} DBSCANState;

/* Compute Euclidean distance squared (faster, avoids sqrt) */
static inline double
dbscan_distance_sq(const float *a, const float *b, int dim)
{
	double sum = 0.0;
	int i;
	for (i = 0; i < dim; i++)
...
static int *
dbscan_region_query(DBSCANState *state, int idx, int *out_count)
{
	int		*neighbors;
	int		capacity = 32;
	int		count = 0;
	int		i;
	double	eps_sq = state->eps * state->eps;

	neighbors = (int *) palloc(sizeof(int) * capacity);

	for (i = 0; i < state->nvec; i++)
	{
		if (dbscan_distance_sq(state->data[idx], state->data[i], state->dim) <= eps_sq)
		{
			if (count >= capacity)
			{
				capacity *= 2;
...
static void
dbscan_expand_cluster(DBSCANState *state, int idx, int *neighbors, int neighbor_count, int cluster_id)
{
	int i;
	int *seed_neighbors;
	int seed_count;
	int current = 0;
	int *seeds;
	int seeds_size = neighbor_count;
	int seeds_capacity = neighbor_count * 2;

	/* Assign all neighbors to this cluster */
	for (i = 0; i < neighbor_count; i++)
		state->labels[neighbors[i]] = cluster_id;

	/* Use neighbors as seeds for expansion */
	seeds = (int *) palloc(sizeof(int) * seeds_capacity);
	memcpy(seeds, neighbors, sizeof(int) * neighbor_count);

	/* Process each seed */
	while (current < seeds_size)
	{
		int seed_idx = seeds[current++];

		/* Find neighbors of this seed */
		seed_neighbors = dbscan_region_query(state, seed_idx, &seed_count);

		/* If seed has enough neighbors, it's a core point */
		if (seed_count >= state->min_pts)
		{
			/* Add unvisited neighbors to cluster */
			for (i = 0; i < seed_count; i++)
			{
				int neighbor_idx = seed_neighbors[i];

				/* If undefined or noise, add to cluster */
				if (state->labels[neighbor_idx] == DBSCAN_UNDEFINED ||
					state->labels[neighbor_idx] == DBSCAN_NOISE)
				{
					if (state->labels[neighbor_idx] == DBSCAN_UNDEFINED)
					{
						/* Add to seeds for further expansion */
						if (seeds_size >= seeds_capacity)
						{
							seeds_capacity *= 2;
							seeds = (int *) repalloc(seeds, sizeof(int) * seeds_capacity);
						}
						seeds[seeds_size++] = neighbor_idx;
					}
					state->labels[neighbor_idx] = cluster_id;
				}
			}
		}

		pfree(seed_neighbors);
	}
