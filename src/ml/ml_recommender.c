...
train_collaborative_filter(PG_FUNCTION_ARGS)
{
    ...
    model_mcxt = AllocSetContextCreate(CurrentMemoryContext, "ALS factors context", ALLOCSET_SMALL_SIZES);
    ...
    /* Model id as current timestamp in ms */
    {
...

        {
            ...
            for (i = 0; i <= max_user_id; ++i)
            {
                ...
                if (!seen)
                    continue;
                array = construct_array((Datum *) P[i], n_factors, FLOAT4OID, sizeof(float4), true, TYPALIGN_INT);
...

        {
            ...
            for (i = 0; i <= max_item_id; ++i)
            {
                ...
                if (!seen)
                    continue;
                array = construct_array((Datum *) Q[i], n_factors, FLOAT4OID, sizeof(float4), true, TYPALIGN_INT);
                values[0] = Int64GetDatum(model_id);
                values[1] = Int32GetDatum(i);
                values[2] = PointerGetDatum(array);
                ret = SPI_execute_plan(plan, values, nulls, false, 1);
...
recommend_items(PG_FUNCTION_ARGS)
{
    ...
    for (i = 0; i < n_items_total; ++i)
    {
		...
		facdatum = SPI_getbinval(itup, SPI_tuptable->tupdesc, 2, &isnull_fac);
...

    {
        ...
        for (i = 0; i < item_count; ++i)
        {
            ...
            if (isnull_item || isnull_feat)
            {
                if (features_table_str) pfree(features_table_str);
                for (j = 0; j < i; ++j)
                    if (other_factors[j]) pfree(other_factors[j]);
...
recommend_hybrid(PG_FUNCTION_ARGS)
{
...

	{
        ...
        for (i = 0; i < n_items_total; ++i)
        {
			...
			id = DatumGetInt32(SPI_getbinval(tup2, desc, 1, &isnull_id));
		...
		content_factors = (float **) palloc(sizeof(float *) * n_feat_items);
		nf_content = 0;
        ...
        for (i = 0; i < n_feat_items; ++i)
        {
            ...
            int id = DatumGetInt32(SPI_getbinval(tupf, descf, 1, &isnull_id));
            Datum arr_tmp = SPI_getbinval(tupf, descf, 2, &isnull_feat);
        ...
        for (i = 0; i < ntop; ++i)
            elems[i] = Int32GetDatum(top_items[i]);
		result_array = construct_array(elems, ntop, INT4OID, sizeof(int32), true, 'i');
        if (top_items) pfree(top_items);
