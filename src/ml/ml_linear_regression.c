...
train_linear_regression(PG_FUNCTION_ARGS)
{
	...
	if (nvec < 10)
	{
		...
		pfree(targ_str);
		ereport(ERROR,
			(errmsg("Need at least 10 samples for linear "
				"regression, have %d",
				nvec)));
	}
	...
	if (neurondb_gpu_is_available() && nvec > 0 && dim > 0)
	{
		...
		{
			elog(DEBUG1,
				"linear_regression: GPU training unavailable, "
				"using CPU");
			if (gpu_err != NULL)
				pfree(gpu_err);
			if (gpu_hyperparams != NULL)
	...
	/* CPU training path */
	{
		...
		if (model->n_features <= 0 || model->n_features > 10000)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("linreg: model.n_features is "
					       "invalid (%d) before "
					       "serialization",
						model->n_features)));
		}
		...
		elog(DEBUG1,
			"linreg: serializing model with n_features=%d, "
			"n_samples=%d",
			model->n_features,
			model->n_samples);
