...
linear_kernel(float *x, float *y, int dim)
{
	...
	{
		elog(ERROR, "linear_kernel: invalid inputs (x=%p, y=%p, dim=%d)",
			(void *)x, (void *)y, dim);
		return 0.0;
	}
	...
	SVMDataset *dataset)
{
	...
	MemoryContextSwitchTo(oldcontext);
	dataset->features = (float *)palloc(sizeof(float) * (size_t)nvec * (size_t)dim);
	...
	/* Extract data */
	{
		...
		if (nvec > 0)
		{
			...
			label_is_int = (label_type == INT4OID || label_type == INT2OID || label_type == INT8OID);
		}
		...
		for (i = 0; i < nvec; i++)
		{
			...
			if (vec->dim != dim)
			{
				...
				ereport(ERROR, (errmsg("Feature dimension mismatch: expected %d, got %d", dim, vec->dim)));
			}
			
			memcpy(dataset->features + i * dim, vec->data, sizeof(float) * dim);
...
svm_model_serialize(const SVMModel *model)
{
	...
	if (model->support_vectors != NULL && model->n_support_vectors > 0 && model->n_features > 0)
	{
		for (i = 0; i < model->n_support_vectors * model->n_features; i++)
			pq_sendfloat4(&buf, model->support_vectors[i]);
	}
	
	if (model->support_vector_indices != NULL && model->n_support_vectors > 0)
	{
		for (i = 0; i < model->n_support_vectors; i++)
...
svm_model_deserialize(const bytea *data)
{
	...
	{
		model->alphas = (double *)palloc(sizeof(double) * (size_t)model->n_support_vectors);
		for (i = 0; i < model->n_support_vectors; i++)
	...
	{
		model->support_vectors = (float *)palloc(sizeof(float) * (size_t)model->n_support_vectors * (size_t)model->n_features);
		for (i = 0; i < model->n_support_vectors * model->n_features; i++)
	...
	if (model->n_support_vectors > 0)
	{
		model->support_vector_indices = (int *)palloc(sizeof(int) * (size_t)model->n_support_vectors);
		for (i = 0; i < model->n_support_vectors; i++)
			model->support_vector_indices[i] = pq_getmsgint(&buf, 4);
	}
...
train_svm_classifier(PG_FUNCTION_ARGS)
{
	...
	if (PG_NARGS() < 3 || PG_NARGS() > 5)
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: train_svm_classifier requires 3-5 arguments, got %d", PG_NARGS()),
				errhint("Usage: train_svm_classifier(table_name, feature_col, label_col, [C], [max_iters])")));
	
	/* Get required arguments */
	if (PG_ARGISNULL(0) || PG_ARGISNULL(1) || PG_ARGISNULL(2))
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: table_name, feature_col, and label_col are required")));
	...
	if (c_param <= 0.0 || c_param > 1000.0)
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: C parameter must be in range (0, 1000], got %.6f", c_param)));
	
	if (max_iters <= 0 || max_iters > 100000)
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: max_iters must be in range (0, 100000], got %d", max_iters)));
	...
	if (nvec < 10)
	{
		...
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: need at least 10 samples for training, got %d", nvec)));
	}
	...
	if (dim <= 0 || dim > 10000)
	{
		...
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("svm: invalid feature dimension %d (must be in range [1, 10000])", dim)));
	}
	...
	/* Validate labels are binary - check for at least two distinct values */
	{
		...
		elog(DEBUG1, "svm: label validation passed (class0=%d, class1=%d, first=%.6f, second=%.6f)",
			n_class0, n_class1, first_label, second_label);
	}
	...
	if (neurondb_gpu_is_available() && nvec > 0 && dim > 0)
	{
		...
		{
			elog(WARNING, "svm: failed to create hyperparameters JSONB, falling back to CPU");
		}
		else
		{
				...
				&& gpu_result.spec.model_data != NULL)
			{
				...
				if (model_id <= 0)
				{
					...
					ereport(ERROR,
						(errcode(ERRCODE_INTERNAL_ERROR),
							errmsg("svm: failed to register GPU model in catalog")));
				}
			...
			{
				if (gpu_err != NULL)
					elog(DEBUG1, "svm: GPU training failed: %s", gpu_err);
				else
					elog(DEBUG1, "svm: GPU training unavailable, falling back to CPU");
				if (gpu_hyperparams != NULL)
				{
					pfree(gpu_hyperparams);
					gpu_hyperparams = NULL;
	...
	elog(DEBUG1, "svm: using CPU training path (n_samples=%d, feature_dim=%d, C=%.6f, max_iters=%d)",
		nvec, dim, c_param, max_iters);
	
	{
		...
		actual_max_iters = (max_iters > 1000 && nvec > 1000) ? 1000 : max_iters;
		sample_limit = (nvec > 5000) ? 5000 : nvec;
		kernel_limit = (sample_limit > 1000) ? 1000 : sample_limit;
		
		elog(DEBUG1, "svm: CPU training limits: actual_max_iters=%d, sample_limit=%d, kernel_limit=%d",
			actual_max_iters, sample_limit, kernel_limit);
		
		/* Allocate memory for SMO */
		alphas = (double *)palloc0(sizeof(double) * (size_t)sample_limit);
		errors = (double *)palloc(sizeof(double) * (size_t)sample_limit);
		...
		if (alphas == NULL || errors == NULL)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_OUT_OF_MEMORY),
					errmsg("svm: failed to allocate memory for SMO algorithm")));
		}
		
		/* Initialize errors: E_i = f(x_i) - y_i, where f(x_i) = 0 initially */
		for (i = 0; i < sample_limit && i < nvec; i++)
		{
			errors[i] = -dataset.labels[i]; /* f(x_i) = 0 initially, so E_i = 0 - y_i = -y_i */
		}
		...
		for (iter = 0; iter < actual_max_iters; iter++)
		{
			...
			if (examine_all)
			{
				for (i = 0; i < sample_limit; i++)
				{
					if (i >= nvec)
						break;
					changed += 1; /* Simplified: always update */
				}
			}
			else
			{
		...
		elog(DEBUG1, "svm: CPU SMO completed after %d iterations, %d support vectors",
			iter, sv_count);
		...
		if (dim <= 0 || dim > 10000)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("svm: invalid feature dimension %d before model serialization", dim)));
		}
		...
		elog(DEBUG1, "svm: building model with n_features=%d, n_samples=%d, sv_count=%d",
			model.n_features, model.n_samples, sv_count);
		
		/* Handle case when no support vectors found */
		if (sv_count == 0)
		{
			elog(WARNING, "svm: no support vectors found, using default model with single support vector");
			sv_count = 1;
			...
			model.support_vectors = (float *)palloc0(sizeof(float) * (size_t)sv_count * (size_t)dim);
			model.support_vector_indices = (int32 *)palloc(sizeof(int32) * (size_t)sv_count);
			...
			if (model.alphas == NULL || model.support_vectors == NULL || model.support_vector_indices == NULL)
			{
				...
				ereport(ERROR,
					(errcode(ERRCODE_OUT_OF_MEMORY),
						errmsg("svm: failed to allocate memory for support vectors")));
			}
			...
			if (nvec > 0 && dim > 0)
			{
				memcpy(model.support_vectors, dataset.features, sizeof(float) * (size_t)dim);
			}
		}
		else
		{
			...
			/* Allocate support vectors and alphas */
			model.alphas = (double *)palloc(sizeof(double) * (size_t)sv_count);
			model.support_vectors = (float *)palloc(sizeof(float) * (size_t)sv_count * (size_t)dim);
			model.support_vector_indices = (int32 *)palloc(sizeof(int32) * (size_t)sv_count);
			...
			{
				int sv_idx = 0;
				for (i = 0; i < sample_limit && i < nvec && sv_idx < sv_count; i++)
				{
					if (alphas[i] > eps)
					{
						model.alphas[sv_idx] = alphas[i];
						model.support_vector_indices[sv_idx] = i;
						memcpy(model.support_vectors + sv_idx * dim,
							dataset.features + i * dim,
							sizeof(float) * dim);
						sv_idx++;
					}
				...
				if (sv_idx != sv_count)
				{
					elog(WARNING, "svm: expected %d support vectors but copied %d", sv_count, sv_idx);
		...
		for (i = 0; i < sample_limit && i < nvec; i++)
		{
			...
			for (j = 0; j < sv_count; j++)
			{
				...
				{
					pred += model.alphas[j] * dataset.labels[sv_idx] *
						linear_kernel(
							model.support_vectors + j * dim,
							dataset.features + i * dim,
							dim);
				}
			}
		...
		accuracy = (sample_limit > 0) ? ((double)correct / (double)sample_limit) : 0.0;
		...
		if (model.n_features <= 0 || model.n_features > 10000)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("svm: model.n_features is invalid (%d) before serialization", model.n_features)));
		}
		
		elog(DEBUG1, "svm: serializing model with n_features=%d, n_support_vectors=%d",
			model.n_features, model.n_support_vectors);
		...
		if (serialized == NULL)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("svm: failed to serialize model")));
		}
		...
		if (params_jsonb == NULL)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("svm: failed to create hyperparameters JSONB")));
		}
		...
		appendStringInfo(&metricsbuf,
			...
			"\"accuracy\":%.6f,"
			"\"bias\":%.6f}",
			nvec, dim, sv_count, c_param, max_iters, iter, accuracy, bias);
		metrics_jsonb = DatumGetJsonbP(DirectFunctionCall1(
			jsonb_in, CStringGetDatum(metricsbuf.data)));
		
		if (metrics_jsonb == NULL)
		{
			elog(WARNING, "svm: failed to create metrics JSONB, continuing without metrics");
		}
		...
		if (model_id <= 0)
		{
			...
			ereport(ERROR,
				(errcode(ERRCODE_INTERNAL_ERROR),
					errmsg("svm: failed to register model in catalog, model_id=%d", model_id)));
		}
		
		elog(DEBUG1, "svm: CPU training completed, model_id=%d", model_id);
...
predict_svm_model_id(PG_FUNCTION_ARGS)
{
	...
	{
		elog(DEBUG1, "svm: GPU prediction succeeded, prediction=%.6f", prediction);
		PG_RETURN_FLOAT8(prediction);
	}
	else
	{
		elog(DEBUG1, "svm: GPU prediction failed or not available, trying CPU");
	}
	...
	for (i = 0; i < model->n_support_vectors; i++)
	{
		...
		prediction += model->alphas[i] * linear_kernel(sv, features->data, features->dim);
	}
