...
cluster_gmm(PG_FUNCTION_ARGS)
{
	...
	elog(DEBUG1, "neurondb: GMM clustering (k=%d, max_iters=%d)", num_components, max_iters);
	...
	for (iter = 0; iter < max_iters; iter++)
	{
		...
		for (i = 0; i < nvec; i++)
		{
			...
			for (k = 0; k < num_components; k++)
			{
				double pdf = gaussian_pdf(data[i], model.means[k], 
										 model.variances[k], dim);
		...
		/* M-step: Update parameters */
		{
			...
			N_k = (double *) palloc0(sizeof(double) * num_components);
			for (k = 0; k < num_components; k++)
		{
			for (i = 0; i < nvec; i++)
	...
	for (i = 0; i < nvec; i++)
		for (k = 0; k < num_components; k++)
			result_datums[i * num_components + k] = Float8GetDatum(responsibilities[i][k]);
