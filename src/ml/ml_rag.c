...
neurondb_chunk_text(PG_FUNCTION_ARGS)
{
	...
	separator = separator_text ? text_to_cstring(separator_text) : pstrdup("\n\n");
	...
	if (input_len == 0)
	{
		...
		result_array = construct_array(chunk_datums, 1, TEXTOID, -1, false, TYPALIGN_INT);
		pfree(chunk_datums);
	...
	while (start < input_len)
	{
		...
		if (separator && strlen(separator) > 0 && end < input_len)
		{
			...
			for (i = end; i > start; i--)
			{
				if (strncmp(input_str + i - strlen(separator), separator, strlen(separator)) == 0)
				{
					sep_pos = i;
					break;
				}
...
neurondb_embed_text(PG_FUNCTION_ARGS)
{
	...
	elog(DEBUG2, "neurondb_embed_text: model='%s', use_gpu=%d", model_name, use_gpu);
	...
	{
		hash = ((hash << 5) + hash) ^ (unsigned char)(model_name[i % strlen(model_name)]);
...
simple_bm25(const char *query, const char *doc)
{
	...
	for (int i = 0; doc_lc[i]; i++) doc_lc[i] = tolower(doc_lc[i]);
...
neurondb_rank_documents(PG_FUNCTION_ARGS)
{
	...
	algorithm = algorithm_text ? text_to_cstring(algorithm_text) : pstrdup("bm25");
...
neurondb_transform_data(PG_FUNCTION_ARGS)
{
	...
	ArrayType  *result_array;
	int			i;
	float8		sum = 0.0, sum_sq = 0.0, min_v = DBL_MAX, max_v = -DBL_MAX, mean, stddev, range;
	...
	deconstruct_array(input_array, element_type, sizeof(float8), FLOAT8PASSBYVAL, 'd',
					  &elem_values, &elem_nulls, &nelems);
	...
	if (strcmp(pipeline_name, "normalize") == 0)
	{
		...
		{
			for (i = 0; i < nelems; i++)
				transformed_data[i] = elem_nulls[i] ? 0.0 : DatumGetFloat8(elem_values[i]) / norm;
		}
	}
	else if (strcmp(pipeline_name, "standardize") == 0)
	{
		...
		{
			for (i = 0; i < nelems; i++)
				transformed_data[i] = elem_nulls[i] ? 0.0 : (DatumGetFloat8(elem_values[i]) - mean) / stddev;
		}
		else
		{
			for (i = 0; i < nelems; i++)
	...
	else if (strcmp(pipeline_name, "min_max") == 0)
	{
		...
		{
			for (i = 0; i < nelems; i++)
				transformed_data[i] = elem_nulls[i] ? 0.0 : (DatumGetFloat8(elem_values[i]) - min_v) / range;
		}
		else
		{
			for (i = 0; i < nelems; i++)
	...
	else
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("unsupported transformation pipeline: \"%s\"", pipeline_name),
	...
	result_array = construct_array(result_datums, nelems, FLOAT8OID, sizeof(float8), FLOAT8PASSBYVAL, 'd');
