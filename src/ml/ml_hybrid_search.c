...
hybrid_score_cmp(const void *a, const void *b)
{
	...
	if (ha->hybrid_score > hb->hybrid_score) return -1;
	if (ha->hybrid_score < hb->hybrid_score) return 1;
	return 0;
}
...
hybrid_search_fusion(PG_FUNCTION_ARGS)
{
	...
	if (ARR_NDIM(doc_ids_array) != 1 ||
		ARR_NDIM(semantic_scores_array) != 1 ||
	...
	if (semantic_weight < 0.0 || semantic_weight > 1.0)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("semantic_weight must be between 0 and 1")));
	...
	elog(DEBUG1, "neurondb: Hybrid search fusion (%d docs, weight=%.2f)", 
		 num_docs, semantic_weight);
	...
	if (normalize)
	{
		...
		/* Normalize to [0, 1] */
		{
			double sem_range = (sem_max - sem_min) < 1e-10 ? 1.0 : (sem_max - sem_min);
			double lex_range = (lex_max - lex_min) < 1e-10 ? 1.0 : (lex_max - lex_min);

			for (i = 0; i < num_docs; i++)
			{
				scores[i].semantic_score = (scores[i].semantic_score - sem_min) / sem_range;
				scores[i].lexical_score = (scores[i].lexical_score - lex_min) / lex_range;
			}
		}
	}
