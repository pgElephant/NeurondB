...
static void get_feature_definitions(int32 store_id, char ***feat_names, char ***feat_types, char ***feat_transforms, int *n_features);
...
neurondb_create_feature_store(PG_FUNCTION_ARGS)
{
    ...
    char   *description = description_text ? text_to_cstring(description_text) : NULL;
    ...
    elog(NOTICE, "neurondb.create_feature_store: name='%s', entity_table='%s', entity_key='%s'",
         store_name, entity_table, entity_key);
    ...
    appendStringInfo(&sql,
                     ...
                     "(store_name, entity_table, entity_key, description, created_at, updated_at) "
    ...
    store_id = DatumGetInt32(SPI_getbinval(SPI_tuptable->vals[0], SPI_tuptable->tupdesc, 1, &isnull));
...
neurondb_register_feature(PG_FUNCTION_ARGS)
{
    ...
    text       *transformation_text = PG_ARGISNULL(3) ? NULL : PG_GETARG_TEXT_PP(3);
    ...
    transformation = transformation_text ? text_to_cstring(transformation_text) : NULL;

    elog(NOTICE, "neurondb.register_feature: store=%d, feature='%s', type=%s, transformation='%s'",
         store_id, feature_name, feature_type, transformation ? transformation : "none");
    ...
    if (transformation)
    {
        appendStringInfo(&sql,
                         ...
                         " store_id, feature_name, feature_type, transformation, created_at) "
    ...
    feature_id = DatumGetInt32(SPI_getbinval(SPI_tuptable->vals[0],
...
neurondb_get_features(PG_FUNCTION_ARGS)
{
    ...
    ArrayType  *features_array = PG_ARGISNULL(2) ? NULL : PG_GETARG_ARRAYTYPE_P(2);
    ...
    if (ARR_NDIM(entities_array) != 1 || ARR_HASNULL(entities_array))
        ereport(ERROR, (errmsg("entity_ids must be a one-dimensional, not null array")));
    entity_elemtype = ARR_ELEMTYPE(entities_array);
    get_typlenbyvalalign(entity_elemtype, &entity_typlen, &entity_typbyval, &entity_typalign);
    deconstruct_array(entities_array, entity_elemtype, entity_typlen, entity_typbyval,
                      entity_typalign, &entity_datums, &entity_nulls, &num_entities);

    /* Features */
    if (features_array)
    {
        if (ARR_NDIM(features_array) != 1 || ARR_HASNULL(features_array))
            ereport(ERROR, (errmsg("feature_names must be a one-dimensional, not null array")));
        feat_elemtype = ARR_ELEMTYPE(features_array);
        get_typlenbyvalalign(feat_elemtype, &feat_typlen, &feat_typbyval, &feat_typalign);
        deconstruct_array(features_array, feat_elemtype, feat_typlen, feat_typbyval,
                          feat_typalign, &feat_datums, &feat_nulls, &num_features);
    ...
    fncontext = AllocSetContextCreate(CurrentMemoryContext, "neurondb_get_features_context", ALLOCSET_DEFAULT_SIZES);
    ...
    for (i = 0; i < num_entities; ++i)
    {
        ...
        appendStringInfo(&sql, "SELECT %s", quote_identifier(entity_key_col));
        for (j = 0; j < ndefs; ++j)
        {
            appendStringInfo(&sql, ", %s", quote_identifier(feature_names[j]));
        }
        appendStringInfo(&sql, " FROM %s WHERE %s = %s",
                         quote_identifier(entity_table), quote_identifier(entity_key_col),
    ...
    if (!o)
        result_jsonb = DatumGetJsonbP(DirectFunctionCall1(jsonb_in, CStringGetDatum("{}")));
    else
    ...
    if (feature_transforms)
    {
        for (j = 0; j < ndefs; ++j)
            if (feature_transforms[j])
                pfree(feature_transforms[j]);
        pfree(feature_transforms);
    }
    pfree(entity_datums);
...
neurondb_feature_engineering(PG_FUNCTION_ARGS)
{
    ...
    for (i = 0; i < ndefs; ++i)
    {
        if (feature_transforms[i] == NULL || strlen(feature_transforms[i]) == 0)
            continue;
        /* Build UPDATE SQL */
        initStringInfo(&sql);
        appendStringInfo(&sql,
                         ...
                         "WHERE dst.%s = src.%s",
                         quote_identifier(entity_table), quote_identifier(feature_names[i]),
                         quote_identifier(entity_key), quote_identifier(entity_key),
                         feature_transforms[i], quote_identifier(source_table),
                         quote_identifier(entity_key), quote_identifier(entity_key));
        ret = SPI_execute(sql.data, false, 0);
        if (ret != SPI_OK_UPDATE)
            ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                        errmsg("Failed to update feature '%s'", feature_names[i])));
...
entity_key_for_store(int32 store_id)
{
    ...
    initStringInfo(&sql);
    appendStringInfo(&sql,
        "SELECT entity_key FROM neurondb.feature_stores WHERE store_id = %d", store_id);

    if (SPI_connect() != SPI_OK_CONNECT)
        ereport(ERROR,
            (errcode(ERRCODE_INTERNAL_ERROR),
             errmsg("could not connect to SPI manager in helper")));
    ...
    if (ret != SPI_OK_SELECT || SPI_processed != 1)
    {
        ...
        ereport(ERROR,
            (errcode(ERRCODE_INTERNAL_ERROR),
             errmsg("feature store %d not found", store_id)));
    }

    entity_key = TextDatumGetCString(SPI_getbinval(SPI_tuptable->vals[0],
        SPI_tuptable->tupdesc, 1, &isnull));
...
entity_table_for_store(int32 store_id)
{
    ...
    initStringInfo(&sql);
    appendStringInfo(&sql,
        "SELECT entity_table FROM neurondb.feature_stores WHERE store_id = %d", store_id);
    ...
    entity_table = TextDatumGetCString(SPI_getbinval(SPI_tuptable->vals[0],
        SPI_tuptable->tupdesc, 1, &isnull));
                        ...
                        int *out_n_features)
{
    ...
    appendStringInfo(&sql,
        ...
        "FROM neurondb.feature_definitions WHERE store_id = %d ORDER BY feature_id", store_id);

    if (SPI_connect() != SPI_OK_CONNECT)
        ereport(ERROR, (errcode(ERRCODE_INTERNAL_ERROR),
                    errmsg("could not connect to SPI manager in feature_definitions helper")));
    ...
    if (ret != SPI_OK_SELECT)
    {
        ...
        ereport(ERROR,
            (errcode(ERRCODE_INTERNAL_ERROR),
             errmsg("could not retrieve feature definitions for store %d", store_id)));
    }
    ...
    for (i = 0; i < nfeat; ++i)
    {
        fnames[i] = TextDatumGetCString(SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 1, &isnull));
        ftypes[i] = TextDatumGetCString(SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 2, &isnull));
        if (SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 3, &isnull) && isnull)
            ftransforms[i] = NULL;
        else
            ftransforms[i] = TextDatumGetCString(SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 3, &isnull));
    }
