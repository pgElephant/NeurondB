...
import_pytorch_model(PG_FUNCTION_ARGS)
{
	...
	char	   *type = model_type ? text_to_cstring(model_type) : pstrdup("classifier");
	StringInfoData result;
	int			model_id;

	/* Validate model type */
	if (strcmp(type, "classifier") != 0 &&
		strcmp(type, "regressor") != 0 &&
		strcmp(type, "transformer") != 0)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("model_type must be 'classifier', 'regressor', or 'transformer'")));
...
import_tensorflow_model(PG_FUNCTION_ARGS)
{
	text	   *model_path = PG_GETARG_TEXT_PP(0);
	text	   *model_name = PG_GETARG_TEXT_PP(1);
	ArrayType  *input_shapes = PG_ARGISNULL(2) ? NULL : PG_GETARG_ARRAYTYPE_P(2);
...
dl_predict(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < n_outputs; i++)
	{
		if (return_probabilities)
			predictions[i] = 1.0f / n_outputs;	/* Uniform distribution */
		else
	...
	result_array = construct_array(elems, n_outputs, FLOAT4OID,
...
export_dl_model(PG_FUNCTION_ARGS)
{
	...
	if (strcmp(format, "onnx") != 0 &&
		strcmp(format, "torchscript") != 0 &&
		strcmp(format, "savedmodel") != 0)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("export_format must be 'onnx', 'torchscript', or 'savedmodel'")));
	...
	initStringInfo(&result);
	appendStringInfo(&result,
					 "Model exported to %s format at: %s",
					 format, path);
...
quantize_dl_model(PG_FUNCTION_ARGS)
{
	...
	char	   *quant_type = quantization_type ? text_to_cstring(quantization_type) : pstrdup("int8");
	StringInfoData result;
	int			quantized_model_id;

	/* Validate */
	if (strcmp(quant_type, "int8") != 0 &&
		strcmp(quant_type, "fp16") != 0 &&
		strcmp(quant_type, "dynamic") != 0)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 errmsg("quantization_type must be 'int8', 'fp16', or 'dynamic'")));
