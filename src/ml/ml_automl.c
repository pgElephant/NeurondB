...
create_ensemble(PG_FUNCTION_ARGS)
{
	...
	n_models = ArrayGetNItems(
		ARR_NDIM(model_ids_array), ARR_DIMS(model_ids_array));
	...
	/* Validate method */
	if (strcmp(method, "voting") != 0 && strcmp(method, "averaging") != 0
		&& strcmp(method, "stacking") != 0)
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				errmsg("method must be 'voting', 'averaging', "
				       "or 'stacking'")));
...
auto_feature_engineering(PG_FUNCTION_ARGS)
{
	...
	n_features = ArrayGetNItems(
		ARR_NDIM(feature_cols_array), ARR_DIMS(feature_cols_array));
