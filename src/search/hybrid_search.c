 ...
 *
 * Implements hybrid search: vector similarity + FTS + metadata/keyword and temporal/diversity/facet.
 * Includes RRF, ColBERT-style multi-vector, and MMR.
 *
 ...
 * (No mixed declarations & code in for-loops or blocks, standard C89 style)
 */
...
hybrid_search(PG_FUNCTION_ARGS)
{
    ...
    Datum       *results_datums = NULL;
    bool        *results_nulls = NULL;
    int          proc, spi_ret, i;
    ...
    /* Build vector literal representation for SQL */
    ...
    appendStringInfo(&sql,
        ...
        vector_weight, vector_weight, limit
    );
    ...
    results_datums = (Datum *) palloc0(sizeof(Datum) * proc);
    results_nulls = (bool *) palloc0(sizeof(bool) * proc);
    for (i = 0; i < proc; ++i)
    {
        bool isnull = false;
    ...
    {
        ArrayType *ret_array = construct_array(results_datums, proc, TEXTOID, -1, false, 'i');
...
reciprocal_rank_fusion(PG_FUNCTION_ARGS)
{
    ...
    int         n_rankers = 0, i = 0, j = 0;
    int16       elmlen;
    bool        elmbyval;
    char        elmalign;
    ArrayType **rank_arrays = NULL;
    int         item_count = 0;
    HTAB       *item_hash;
    HASHCTL     info;
    ...
    for (i = 0; i < n_rankers; i++)
    {
        ...
        ranker = rank_arrays[i];
        count = ArrayGetNItems(ARR_NDIM(ranker), ARR_DIMS(ranker));
        elemtype = ARR_ELEMTYPE(ranker);
        get_typlenbyvalalign(elemtype, &elmlen, &elmbyval, &elmalign);

        /* -- fix only this: scope for ids/nulls moved up to avoid mixed decl/code -- */
        {
            Datum *ids = NULL;
            bool *nulls = NULL;
            int cc;
    ...
    result_datums = (Datum *) palloc0(sizeof(Datum) * item_count);
    result_nulls = (bool *) palloc0(sizeof(bool) * item_count);
    ...
    /* TODO: Sort result_datums by decreasing RRF score (omitted; order is deterministic on same input) */
...
semantic_keyword_search(PG_FUNCTION_ARGS)
{
    ...
    datums = (Datum *) palloc0(sizeof(Datum) * proc);
    nulls = (bool *) palloc0(sizeof(bool) * proc);
    for (i = 0; i < proc; ++i)
    {
        bool isnull = false;
        Datum val = SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 1, &isnull);
    ...
    for (i = 0; i < nvecs; ++i)
    {
            ...
            continue;
        {
            ...
            for (k = 0; k < qv->dim; ++k)
            {
                ...
                if (k)
                    appendStringInfoChar(&lit, ',');
                appendStringInfo(&lit, "%g", ((float *)&qv[1])[k]);
                if (k == qv->dim-1)
                    appendStringInfoChar(&lit, '}');
            }
 ...
 * (No mixed declarations and code within loops/blocks)
 */
...
diverse_vector_search(PG_FUNCTION_ARGS)
{
    ...
    char *tbl_str = text_to_cstring(table_name);
    StringInfoData sql, vec_lit;
    int i, j, spi_ret, proc, n_candidates, select_count = 0;
    ArrayType *ret_array;
    Datum *datums;
    bool *nulls;
    ...
    /* Get candidates with their relevance */
    initStringInfo(&sql);
    appendStringInfo(
        &sql,
    ...
    /* fix only this: don't do mixed declarations; move typedef up */
    {
        typedef struct mmr_cand_t_s {
        char *id;
        Vector *vec;
        float rel;
        bool selected;
    } mmr_cand_t;
    mmr_cand_t *cands = (mmr_cand_t *) palloc0(sizeof(mmr_cand_t) * n_candidates);
    ...
    for (i = 0; i < n_candidates; ++i)
    {
        ...
        isnull2 = false;
        isnull3 = false;
        id_d = SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 1, &isnull1);
        vec_d = SPI_getbinval(SPI_tuptable->vals[i], SPI_tuptable->tupdesc, 2, &isnull2);
        {
    ...
    datums = (Datum *) palloc0(sizeof(Datum) * top_k);
    nulls = (bool *) palloc0(sizeof(bool) * top_k);
    ...
    for (i = 0; i < top_k && select_count < n_candidates; ++i)
    {
        ...
        for (j = 0; j < n_candidates; ++j)
        {
                ...
                for (s = 0; s < i; ++s)
            {
                if (nulls[s]) continue;
                {
                    Vector *selv = cands[j].vec;
                    Vector *other = ((mmr_cand_t *)cands)[s].vec;
                    float dot = 0;
                    int dim = selv->dim;
                    const float *x = (const float *)&selv[1];
                    const float *y = (const float *)&other[1];
                    for (d = 0; d < dim; ++d)
                        dot += x[d] * y[d];
            ...
            {
                float mmr_score = lambda * cands[j].rel - (1.0 - lambda) * max_diverse;
    ...
    {
        pfree(cands[i].id);
        /* Only free detoasted vectors that are not pointing at SPI tuple memory */
