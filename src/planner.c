...
learn_from_query(PG_FUNCTION_ARGS)
{
	...
	elog(DEBUG1, "neurondb: learning from query (fingerprint=%" UINT64_FORMAT ", recall=%.4f, latency=%d)",
	...
	/* Try SELECT first to see if fingerprint exists */
	{
		...
		appendStringInfo(&sql,
			"SELECT fingerprint, seen_count, ef_search, beam_size FROM neurondb_query_history WHERE fingerprint = %" UINT64_FORMAT, fingerprint);
	...
	if (found)
	{
...

		{
			...
			appendStringInfo(&usql,
				"UPDATE neurondb_query_history SET last_recall=%.6f, last_latency=%d, seen_count=%d, ef_search=%d, beam_size=%d WHERE fingerprint=%" UINT64_FORMAT,
	...
	else
	{
		...
		appendStringInfo(&isql,
			...
			"VALUES (%" UINT64_FORMAT ", %.6f, %d, %d, %d, %d)",
