...
{
    uint32 hash = hash_token(token);
    for (VocabEntry *entry = vocab_table[hash]; entry != NULL; entry = entry->next)
    {
...
vocab_lookup_id(VocabEntry **vocab_table, int32 token_id)
{
    ...
    {
        for (VocabEntry *entry = vocab_table[i]; entry != NULL; entry = entry->next)
        {
...
evict_lru_tokenizer(void)
{
    ...
    prev = NULL;
    for (entry = g_tokenizer_cache_head; entry; prev = entry, entry = entry->next)
    {
    ...
    if (lru_entry->vocab_table && lru_entry->vocab_table != g_default_vocab_table)
    {
        int32 i;
        for (i = 0; i < VOCAB_HASH_SIZE; i++)
        {
            VocabEntry *vocab_entry, *next;
            for (vocab_entry = lru_entry->vocab_table[i]; vocab_entry; vocab_entry = next)
            {
...
get_or_load_tokenizer(const char *model_name)
{
    ...
    entry = (TokenizerCacheEntry *) MemoryContextAllocZero(TopMemoryContext, sizeof(TokenizerCacheEntry));
    entry->model_name = MemoryContextStrdup(TopMemoryContext, model_name);
    entry->vocab_table = (VocabEntry **) MemoryContextAllocZero(TopMemoryContext, VOCAB_HASH_SIZE * sizeof(VocabEntry *));
    entry->last_used = time(NULL);

    /* Try to load vocabulary from model directory */
    if (neurondb_onnx_model_path)
    {
        snprintf(vocab_path, MAXPGPATH, "%s/%s/vocab.txt", neurondb_onnx_model_path, model_name);
        
        /* Check if vocab file exists */
        if (access(vocab_path, R_OK) == 0)
        {
            entry->vocab_path = MemoryContextStrdup(TopMemoryContext, vocab_path);
            vocab_size = load_vocab_file(entry->vocab_table, vocab_path);
...
{
    return neurondb_tokenize_with_model(text, max_length, output_length, NULL);
}
 ...
 */
int32 *
neurondb_tokenize_with_model(const char *text, int32 max_length, int32 *output_length, const char *model_name)
{
 ...
 */
char *
neurondb_detokenize(const int32 *token_ids, int32 length, const char *model_name)
{
    StringInfoData buf;
    int32 i;
    TokenizerCacheEntry *tokenizer = NULL;
    ...
    for (i = 0; i < length; i++)
    {
        ...
        if (token_id == TOKEN_PAD_ID || token_id == TOKEN_CLS_ID || 
            token_id == TOKEN_SEP_ID || token_id == TOKEN_MASK_ID)
        {
            continue;
        }
...
neurondb_hf_tokenize(PG_FUNCTION_ARGS)
{
    ...
    {
        arg0_type = get_fn_expr_argtype(fcinfo->flinfo, 0);
    ...
    else
    {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("neurondb_hf_tokenize: invalid number of arguments")));
    }
    ...
    if (max_length < 2 || max_length > MAX_SEQ_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("max_length must be between 2 and %d", MAX_SEQ_LENGTH)));

    /* Tokenize text */
    token_ids = neurondb_tokenize_with_model(text_str, max_length, &output_length, model_name);
    ...
    /* Create array result */
    result = construct_array(dvalues, output_length, INT4OID, sizeof(int32), true, 'i');
...
neurondb_hf_detokenize(PG_FUNCTION_ARGS)
{
    ...
    else
    {
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("neurondb_hf_detokenize: invalid number of arguments")));
    }
    ...
    if (ndims != 1)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("token_ids must be a one-dimensional array")));

    if (ARR_ELEMTYPE(token_ids_array) != INT4OID)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("token_ids must be an array of integers")));

    length = ARR_DIMS(token_ids_array)[0];
    if (length < 0 || length > MAX_SEQ_LENGTH)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                 errmsg("token_ids array length must be between 0 and %d", MAX_SEQ_LENGTH)));

    /* Extract array values */
    deconstruct_array(token_ids_array, INT4OID, sizeof(int32), true, 'i',
                      &dvalues, &dnulls, &length);
