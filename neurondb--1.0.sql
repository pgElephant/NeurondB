...
-- ============================================================================
-- PostgreSQL Version: 16-18 (160000-180000)
-- OS/Platform: macOS (Darwin)
-- Build Date: 2025-11-06 21:25:08
-- Build Type: macos_pg16
-- ============================================================================
...
-- Single prediction (OLD API - replaced by unified API below)
...
DECLARE
    ...
    seed_val float4;
BEGIN
    -- Generate deterministic 384-dimensional embedding based on text hash
    seed_val := hashtext(text) / 2147483647.0;
    
    FOR i IN 1..384 LOOP
        val := ((seed_val + i * 0.1) * 1000)::int % 2000 / 1000.0 - 1.0;
        embedding_array := array_append(embedding_array, val);
        norm := norm + (val * val);
    END LOOP;
    ...
    IF should_normalize THEN
        norm := sqrt(norm);
        FOR i IN 1..384 LOOP
            embedding_array[i] := embedding_array[i] / norm;
        END LOOP;
    END IF;
    
    RETURN array_to_string(embedding_array, ',')::vector(384);
END;
$$;
...
-- Alias for compatibility with demos
CREATE OR REPLACE FUNCTION neurondb_generate_embedding(model TEXT, text TEXT) 
RETURNS vector(384)
LANGUAGE sql IMMUTABLE
AS $$
    SELECT neurondb.generate_embedding(model, text, true);
$$;
COMMENT ON FUNCTION neurondb_generate_embedding IS 'Generate 384-dim embeddings (alias for neurondb.generate_embedding)';
...
-- neurondb.predict() grant moved to unified API section
GRANT EXECUTE ON FUNCTION neurondb.predict_proba(TEXT, ANYARRAY) TO PUBLIC;
-- neurondb.predict_batch() grant moved to unified API section (old version deprecated)
...
-- neurondb.model_info(TEXT) deprecated, grant moved to unified API section
...
RETURNS TABLE(entity TEXT, entity_type TEXT, confidence FLOAT8, start_pos INTEGER)
...
-- Convenience wrappers with neurondb_ prefix
CREATE FUNCTION neurondb_train_xgboost(
    table_name text,
    feature_col text,
    label_col text,
    params jsonb DEFAULT '{}'::jsonb
...
LANGUAGE plpgsql AS $$
DECLARE
    n_estimators integer := COALESCE((params->>'n_estimators')::integer, 100);
    max_depth integer := COALESCE((params->>'max_depth')::integer, 6);
    lr float := COALESCE((params->>'learning_rate')::float, 0.3);
BEGIN
    RETURN train_xgboost_classifier(table_name, feature_col, label_col, n_estimators, max_depth, lr);
END;
$$;
COMMENT ON FUNCTION neurondb_train_xgboost IS 'Train XGBoost with JSON params: (table, features, labels, params)';

CREATE FUNCTION neurondb_predict_xgboost(model_id integer, features real[])
RETURNS float8
LANGUAGE sql STABLE STRICT AS $$
    SELECT predict_xgboost(model_id, features);
$$;
COMMENT ON FUNCTION neurondb_predict_xgboost IS 'Predict with XGBoost model';

-- =============================================================================
-- AutoML - Complete C Implementation
-- =============================================================================
...
CREATE FUNCTION auto_train(
    ...
    LANGUAGE C STABLE;
COMMENT ON FUNCTION auto_train IS 'Auto train best model: (table, features, labels)';

CREATE FUNCTION optimize_hyperparameters(
    algorithm text,
    table_name text,
    params jsonb
) RETURNS jsonb
    AS 'MODULE_PATHNAME', 'optimize_hyperparameters'
    LANGUAGE C STABLE;
COMMENT ON FUNCTION optimize_hyperparameters IS 'Hyperparameter optimization';

CREATE FUNCTION cross_validate(
    algorithm text,
    table_name text,
    feature_col text,
    label_col text,
    ...
    AS 'MODULE_PATHNAME', 'cross_validate'
    LANGUAGE C STABLE;
COMMENT ON FUNCTION cross_validate IS 'K-fold cross-validation';

CREATE FUNCTION model_leaderboard()
RETURNS TABLE(algorithm text, score float8, params jsonb)
    AS 'MODULE_PATHNAME', 'model_leaderboard'
    LANGUAGE C STABLE;
COMMENT ON FUNCTION model_leaderboard IS 'Show model leaderboard';

-- Convenience wrapper
CREATE OR REPLACE FUNCTION neurondb_automl_search(
    table_name text,
    feature_col text,
    label_col text,
    algorithms text[],
...
DECLARE
    result_id integer;
BEGIN
    result_id := auto_train(table_name, feature_col, label_col);
    RETURN jsonb_build_object('model_id', result_id, 'status', 'complete');
END;
$$;
COMMENT ON FUNCTION neurondb_automl_search IS 'AutoML model selection wrapper';

-- =============================================================================
-- Neural Network / Deep Learning - Complete C Implementation  
-- =============================================================================

CREATE FUNCTION train_neural_network(
    table_name text,
    feature_col text,
    label_col text,
    layers integer[],
    ...
    AS 'MODULE_PATHNAME', 'train_neural_network'
    LANGUAGE C STABLE;
COMMENT ON FUNCTION train_neural_network IS 'Train Neural Network: (table, features, labels, layers, learning_rate)';

CREATE FUNCTION predict_neural_network(
    model_id integer,
    features float8[]
) RETURNS float8
    AS 'MODULE_PATHNAME', 'predict_neural_network'
    LANGUAGE C STABLE STRICT;
COMMENT ON FUNCTION predict_neural_network IS 'Predict with Neural Network: (model_id, features)';

-- Convenience wrapper
CREATE OR REPLACE FUNCTION neurondb_train_neural_network(
    table_name text,
    feature_col text,
    label_col text,
    params jsonb DEFAULT '{}'::jsonb
...
LANGUAGE plpgsql AS $$
DECLARE
    layers_str text := COALESCE(params->>'layers', '[10,5,1]');
    layers integer[];
    lr float8 := COALESCE((params->>'learning_rate')::float8, 0.01);
BEGIN
    layers := string_to_array(trim(both '[]' from layers_str), ',')::integer[];
    RETURN train_neural_network(table_name, feature_col, label_col, layers, lr);
END;
$$;
COMMENT ON FUNCTION neurondb_train_neural_network IS 'Train Neural Network with JSON params';

CREATE OR REPLACE FUNCTION neurondb_predict_neural_network(
    model_id integer,
    features vector
) RETURNS float8
LANGUAGE plpgsql AS $$
...
END;
$$;
COMMENT ON FUNCTION neurondb_predict_neural_network IS 'Predict with Neural Network wrapper';
...
-- ============================================================================
-- ONNX RUNTIME & HUGGINGFACE INTEGRATION
-- ============================================================================

-- ONNX Runtime information function
CREATE FUNCTION neurondb_onnx_info()
    RETURNS TEXT
    AS 'MODULE_PATHNAME', 'neurondb_onnx_info'
    LANGUAGE C STRICT;
COMMENT ON FUNCTION neurondb_onnx_info IS 'Get ONNX Runtime version and availability';

-- HuggingFace model functions (ONNX backend)
CREATE FUNCTION neurondb_hf_embedding(model_name TEXT, input_text TEXT)
    RETURNS vector
    AS 'MODULE_PATHNAME', 'neurondb_hf_embedding'
    LANGUAGE C STRICT;
COMMENT ON FUNCTION neurondb_hf_embedding IS 'Generate embeddings using HuggingFace model via ONNX';

CREATE FUNCTION neurondb_hf_classify(model_name TEXT, input_text TEXT)
    RETURNS TEXT
    AS 'MODULE_PATHNAME', 'neurondb_hf_classify'
    LANGUAGE C STRICT;
COMMENT ON FUNCTION neurondb_hf_classify IS 'Text classification using HuggingFace model via ONNX';

CREATE FUNCTION neurondb_hf_ner(model_name TEXT, input_text TEXT)
    RETURNS TEXT
    AS 'MODULE_PATHNAME', 'neurondb_hf_ner'
    LANGUAGE C STRICT;
COMMENT ON FUNCTION neurondb_hf_ner IS 'Named Entity Recognition using HuggingFace model via ONNX';

CREATE FUNCTION neurondb_hf_qa(model_name TEXT, question TEXT, context TEXT)
    RETURNS TEXT
    AS 'MODULE_PATHNAME', 'neurondb_hf_qa'
    LANGUAGE C STRICT;
COMMENT ON FUNCTION neurondb_hf_qa IS 'Question Answering using HuggingFace model via ONNX';

-- ONNX Runtime status view
CREATE VIEW neurondb.onnx_runtime_status AS
SELECT 
    neurondb_onnx_info() AS info,
    CASE 
        WHEN neurondb_onnx_info()::jsonb->>'available' = 'true' THEN 'Available'
        ELSE 'Not Available'
    END AS status,
    (neurondb_onnx_info()::jsonb->>'version')::text AS version;
COMMENT ON VIEW neurondb.onnx_runtime_status IS 'ONNX Runtime availability and version';
...
END;
$$;
COMMENT ON FUNCTION neurondb.embed IS 'Unified embedding function: embed(model, text, task) - PostgresML compatible';
...
-- Grant permissions on unified API
GRANT EXECUTE ON FUNCTION neurondb.train TO PUBLIC;
GRANT EXECUTE ON FUNCTION neurondb.predict TO PUBLIC;
...
-- =============================================================================
-- CONVENIENCE ALIASES (PostgresML Compatibility)
-- =============================================================================

-- Create pgml schema for PostgresML compatibility
CREATE SCHEMA IF NOT EXISTS pgml;

-- pgml.embed() compatibility
...
LANGUAGE sql STABLE AS $$
    SELECT neurondb.embed(transformer, text_input);
$$;
COMMENT ON FUNCTION pgml.embed IS 'PostgresML compatible: pgml.embed(model, text) - alias for neurondb.embed()';

-- pgml.train() compatibility
...
DECLARE
    model_id integer;
BEGIN
    -- Map PostgresML task to NeuronDB algorithm
    model_id := neurondb.train(
        COALESCE(algorithm, 'linear_regression'),
        relation_name,
        'features',  -- Assume standard column name
        y_column_name,
        hyperparams
    );
    
    RETURN 'Model trained: ' || model_id::text;
END;
$$;
COMMENT ON FUNCTION pgml.train IS 'PostgresML compatible: pgml.train() - alias for neurondb.train()';
...
GRANT EXECUTE ON FUNCTION neurondb.predict_batch TO PUBLIC;
...
-- PostgresML compatibility
GRANT USAGE ON SCHEMA pgml TO PUBLIC;
GRANT EXECUTE ON FUNCTION pgml.embed TO PUBLIC;
GRANT EXECUTE ON FUNCTION pgml.train TO PUBLIC;
