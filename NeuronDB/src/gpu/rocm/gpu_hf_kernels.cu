 ...
 *
 * gpu_hf_kernels.cu
 *	  ROCm kernels for Hugging Face / LLM operations.
 *
...
ndb_rocm_hf_tokenize_simple_kernel(const char *text,
	int text_len,
	int32_t *token_ids,
...
ndb_rocm_hf_embedding_lookup_kernel(const float *embedding_table,
 ...
 */
__global__ static void
ndb_rocm_hf_single_embedding_lookup_kernel(const float *embedding_table,
	int32_t token_id,
	float *embedding,
	int embed_dim,
	int vocab_size)
 ...
 */
__global__ static void
ndb_rocm_hf_add_position_embedding_kernel(const float *token_embedding,
 ...
 */
__global__ static void
ndb_rocm_hf_add_vectors_kernel(const float *a,
	const float *b,
	float *output,
	int n)
{
 ...
 */
__global__ static void __attribute__((unused))
ndb_rocm_hf_multiply_vectors_kernel(const float *a,
	const float *b,
	float *output,
	int n)
{
 ...
 */
__global__ static void
ndb_rocm_hf_mean_pooling_kernel(const float *embeddings,
	const int32_t *attention_mask,
 ...
 */
__global__ static void __attribute__((unused))
ndb_rocm_hf_cls_pooling_kernel(const float *embeddings,
	float *pooled_embedding,
	int embed_dim)
{
	int dim_idx = blockIdx.x * blockDim.x + threadIdx.x;
...
/*======================================================================*/
/* Matrix Operations (using rocBLAS) */
/*======================================================================*/

/*
 * Matrix multiplication wrapper using rocBLAS
 */
static rocblas_handle g_rocblas_handle = NULL;

static int
ndb_rocm_init_rocblas(void)
{
	rocblas_status status;

	if (g_rocblas_handle != NULL)
		return 0;

	status = rocblas_create_handle(&g_rocblas_handle);
 ...
 * rocBLAS uses column-major order, so we need to handle row-major inputs
 */
static int __attribute__((unused))
ndb_rocm_matmul(const float *A, const float *B, float *C, int m, int n, int k)
{
	rocblas_status status;
	const float alpha = 1.0f;
	const float beta = 0.0f;

	if (ndb_rocm_init_rocblas() != 0)
		return -1;

	/* rocBLAS uses column-major, so for row-major A * B:
	 * C = A * B (row-major) = (B^T * A^T)^T (column-major)
	 * We use: C^T = B^T * A^T
	 * So: C = (B^T * A^T)^T
	 */
	status = rocblas_sgemm(g_rocblas_handle,
		rocblas_operation_transpose, /* Transpose B (B^T) */
		rocblas_operation_transpose, /* Transpose A (A^T) */
 ...
 */
static int
ndb_rocm_matvec(const float *A, const float *x, float *y, int m, int n)
{
	rocblas_status status;
	const float alpha = 1.0f;
	const float beta = 0.0f;
	...
	/* rocBLAS gemv: y = alpha * A * x + beta * y */
	/* A is column-major [m x n], x is [n], y is [m] */
	status = rocblas_sgemv(g_rocblas_handle,
 ...
 */
__global__ static void
ndb_rocm_hf_compute_logits_kernel(const float *hidden_states,
 ...
 */
__global__ static void
ndb_rocm_hf_layer_norm_kernel(const float *input,
	const float *gamma,
	const float *beta,
	float *output,
	int seq_len,
 ...
 */
__global__ static void
ndb_rocm_hf_gelu_kernel(const float *input, float *output, int n)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 */
__global__ static void __attribute__((unused))
ndb_rocm_hf_softmax_kernel(const float *input,
	float *output,
	int seq_len,
	int head_dim)
{
 ...
 */
extern "C" int
ndb_rocm_hf_embed_inference(const char *model_name,
	const int32_t *token_ids,
	const int32_t *attention_mask,
	int seq_len,
	...
	char **errstr)
{
	...
	status = hipMalloc((void **)&d_token_ids, token_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_attention_mask, mask_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_embedding_table, embed_table_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_embeddings, embeddings_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_pooled, pooled_bytes);
	if (status != hipSuccess)
		goto error;

	/* Copy input data to device */
	status = hipMemcpy(
		d_token_ids, token_ids, token_bytes, hipMemcpyHostToDevice);
	if (status != hipSuccess)
		goto error;
	status = hipMemcpy(d_attention_mask,
		attention_mask,
		mask_bytes,
		hipMemcpyHostToDevice);
	if (status != hipSuccess)
		goto error;
	status = hipMemcpy(d_embedding_table,
		embedding_table,
		embed_table_bytes,
		hipMemcpyHostToDevice);
	if (status != hipSuccess)
	...
	ndb_rocm_hf_embedding_lookup_kernel<<<blocks, threads>>>(
		d_embedding_table,
		d_token_ids,
		d_attention_mask,
		d_embeddings,
	...
	ndb_rocm_hf_mean_pooling_kernel<<<blocks, threads>>>(
	...
	status = hipMemcpy(output_embedding,
		d_pooled,
		pooled_bytes,
...
error:
	...
	if (errstr && !*errstr)
	{
		const char *err_msg = hipGetErrorString(status);
 ...
 */
__global__ static void
ndb_rocm_hf_causal_attention_kernel(const float *query,
...
/*
 * Compute Q, K, V from hidden states using rocBLAS
 ...
 */
static int
ndb_rocm_hf_compute_qkv(const float *hidden_states,
	...
	int head_dim)
{
	...
	rc = ndb_rocm_matvec(key_weights, hidden_states, k, qkv_dim, embed_dim);
	if (rc != 0)
		return -1;
...
/*
 * Apply attention output projection using rocBLAS
 ...
 */
static int
ndb_rocm_hf_attention_output_projection(const float *attention_output,
	const float *output_weights,
	float *output,
	int embed_dim)
{
	return ndb_rocm_matvec(
		output_weights, attention_output, output, embed_dim, embed_dim);
}

/*
 * Apply feed-forward network using rocBLAS
 ...
 */
static int
ndb_rocm_hf_ffn_forward(const float *input,
	...
	int hidden_dim)
{
	...
	status = hipMalloc((void **)&d_gelu_input, sizeof(float) * hidden_dim);
	if (status != hipSuccess)
		return -1;
	...
	/* Apply GELU activation */
	{
		...
		ndb_rocm_hf_gelu_kernel<<<blocks, threads>>>(
 ...
 */
__global__ static void __attribute__((unused))
ndb_rocm_hf_ffn_kernel(const float *input,
	const float *ffn_weights1,
	const float *ffn_weights2,
	float *output,
	int embed_dim,
	int hidden_dim)
{
	...
	/* Compute FFN output (simplified - should use rocBLAS for matrix multiplication) */
 ...
 */
__global__ static void __attribute__((unused))
ndb_rocm_hf_update_kv_cache_kernel(const float *new_key,
 ...
 */
__global__ static void
ndb_rocm_hf_apply_logit_bias_kernel(float *logits,
 ...
 */
__global__ static void
ndb_rocm_hf_temperature_scale_kernel(float *logits,
	float temperature,
	int vocab_size)
{
	int tid = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 */
__global__ static void
ndb_rocm_hf_find_top_k_kernel(const float *logits,
	int *top_k_indices,
	float *top_k_values,
	int top_k,
	int vocab_size)
 ...
 */
__global__ static void
ndb_rocm_hf_top_k_filter_kernel(float *logits,
 ...
 */
__global__ static void
ndb_rocm_hf_top_p_filter_kernel(float *logits,
 ...
 */
__global__ static void
ndb_rocm_hf_sort_logits_kernel(const float *logits,
	float *sorted_logits,
	int *sorted_indices,
	int vocab_size)
{
 ...
 */
__global__ static void
ndb_rocm_hf_repetition_penalty_kernel(float *logits,
 ...
 */
__global__ static void
ndb_rocm_hf_greedy_sample_kernel(const float *logits,
 ...
 */
__global__ static void
ndb_rocm_hf_multinomial_sample_kernel(const float *logits,
	float *probs,
	float *cumsum,
	float random_value,
	int32_t *output_token,
 ...
 */
__global__ static void
ndb_rocm_hf_check_stop_sequence_kernel(const int32_t *generated_tokens,
	int num_generated,
...
/*
 * ndb_rocm_hf_generate_inference
 ...
 */
extern "C" int
ndb_rocm_hf_generate_inference(const char *model_name,
	...
	char **errstr)
{
	...
	hiprandState *d_curand_state = NULL;
	...
	status = hipMalloc((void **)&d_embeddings, embed_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_logits, logit_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_probs, logit_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_cumsum, logit_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_input_tokens, token_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_output_tokens, token_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_key_cache, cache_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_value_cache, cache_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_found_stop, sizeof(bool));
	if (status != hipSuccess)
		goto error;

	/* Copy input tokens to device */
	status = hipMemcpy(d_input_tokens,
		input_token_ids,
	...
	status = hipMemset(d_logits, 0, logit_bytes);
	if (status != hipSuccess)
		goto error;
	...
	for (i = 0; i < max_gen_tokens && !stop_found; i++)
	{
		...
		status =
			hipMalloc((void **)&d_current_embedding, hidden_bytes);
		if (status != hipSuccess)
		{
			generated_count =
		...
		status = hipMalloc((void **)&d_hidden_states, hidden_bytes);
		if (status != hipSuccess)
		{
		...
		status = hipMalloc((void **)&d_ffn_output, ffn_output_bytes);
		if (status != hipSuccess)
		{
			hipFree(d_current_embedding);
			hipFree(d_hidden_states);
		...
		ndb_rocm_hf_single_embedding_lookup_kernel<<<blocks, threads>>>(
			embedding_table,
			current_token_id,
			d_current_embedding,
			config->embed_dim,
		...
		if (position_embeddings != NULL)
		{
			...
			if (pos >= 0 && pos < config->max_seq_len)
			{
				...
				status = hipMalloc((void **)&d_pos_embedding,
				...
				status = hipMemcpy(d_pos_embedding,
					position_embeddings
				...
				ndb_rocm_hf_add_position_embedding_kernel<<<
					blocks,
			...
			} else
			{
				...
				status = hipMemcpy(d_hidden_states,
					d_current_embedding,
					sizeof(float) * config->embed_dim,
					hipMemcpyDeviceToDevice);
				if (status != hipSuccess)
		...
		/* Run full transformer layer forward pass for each layer */
		{
			...
			status = hipMalloc((void **)&d_q, qkv_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_layer_output);
				hipFree(d_current_embedding);
				hipFree(d_hidden_states);
				hipFree(d_attention_output);
				hipFree(d_ffn_output);
			...
			status = hipMalloc((void **)&d_k, qkv_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_q);
				hipFree(d_layer_output);
			...
			status = hipMalloc((void **)&d_v, qkv_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_k);
				hipFree(d_q);
			...
			status =
				hipMalloc((void **)&d_attn_output, attn_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_v);
				hipFree(d_k);
			...
			status = hipMalloc((void **)&d_ffn_input, layer_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_attn_output);
				hipFree(d_v);
				hipFree(d_k);
				hipFree(d_q);
				hipFree(d_layer_output);
			...
			status = hipMalloc((void **)&d_residual, layer_bytes);
			if (status != hipSuccess)
			{
				hipFree(d_ffn_output_temp);
				hipFree(d_ffn_input);
				hipFree(d_attn_output);
				hipFree(d_v);
				hipFree(d_k);
			...
			for (layer = 0; layer < config->num_layers; layer++)
			{
				...
				if (status != hipSuccess)
				{
					hipFree(d_layer_norm_input);
					break;
				}

				/* Copy input to layer norm buffer */
				status = hipMemcpy(d_layer_norm_input,
					d_layer_input,
					sizeof(float) * config->embed_dim,
					hipMemcpyDeviceToDevice);
				if (status != hipSuccess)
				{
					hipFree(d_layer_norm_output);
					hipFree(d_layer_norm_input);
					break;
				}
					...
					&& weights->layer_norm_beta != NULL)
				{
					...
					ndb_rocm_hf_layer_norm_kernel<<<blocks,
				...
				} else
				{
					...
					status = hipMemcpy(d_layer_norm_output,
						d_layer_norm_input,
						sizeof(float)
							* config->embed_dim,
						hipMemcpyDeviceToDevice);
				...
				if (kv_cache != NULL && kv_cache->allocated)
				{
					...
					/* Copy K, V to cache */
					status = hipMemcpy(layer_key_cache,
						d_k,
						sizeof(float)
							* config->num_heads
							* head_dim,
					...
					status = hipMemcpy(layer_value_cache,
						d_v,
						sizeof(float)
							* config->num_heads
							* head_dim,
				...
				ndb_rocm_hf_causal_attention_kernel<<<blocks,
					threads>>>(d_q,
				...
				if (weights->output_weights != NULL)
				{
					...
					rc = ndb_rocm_hf_attention_output_projection(
						d_attn_output,
					...
					status = hipMemcpy(d_attn_output,
						d_attn_proj,
						sizeof(float)
							* config->embed_dim,
						hipMemcpyDeviceToDevice);
				...
				ndb_rocm_hf_add_vectors_kernel<<<blocks,
					threads>>>(d_residual,
					d_attn_output,
					d_ffn_input,
					config->embed_dim);
		...
		if (lm_head_weights != NULL)
		{
			...
			ndb_rocm_hf_compute_logits_kernel<<<blocks, threads>>>(
				d_hidden_states,
				lm_head_weights,
				d_logits,
				config->embed_dim,
		...
		if (gen_params->num_logit_bias > 0)
		{
			...
			if (status == hipSuccess)
			{
				status = hipMalloc((void **)&d_bias_values,
					bias_values_bytes);
				if (status == hipSuccess)
				{
					/* Copy bias tokens and values to device */
					status = hipMemcpy(d_bias_tokens,
					...
					if (status == hipSuccess)
					{
						...
						{
							/* Apply logit bias */
							ndb_rocm_hf_apply_logit_bias_kernel<<<
								blocks,
								threads>>>(
								d_logits,
								d_bias_tokens,
						...
						{
							hipFree(d_bias_tokens);
							hipFree(d_bias_values);
							generated_count = i;
							break;
						}
					} else
		...
		{
			ndb_rocm_hf_temperature_scale_kernel<<<blocks,
				threads>>>(d_logits,
			...
			&& gen_params->top_k < config->vocab_size)
		{
			...
			if (status == hipSuccess)
			{
				status = hipMalloc((void **)&d_top_k_values,
					top_k_values_bytes);
				if (status == hipSuccess)
				{
					/* Find top-k indices */
					ndb_rocm_hf_find_top_k_kernel<<<blocks,
						threads>>>(d_logits,
						d_top_k_indices,
						d_top_k_values,
						gen_params->top_k,
					...
					if (status == hipSuccess)
					{
						...
						if (status == hipSuccess)
						{
							...
							ndb_rocm_hf_top_k_filter_kernel<<<
								blocks,
								threads>>>(
								d_logits,
								d_top_k_indices,
		...
		if (gen_params->top_p > 0.0f && gen_params->top_p < 1.0f)
		{
			...
			if (status == hipSuccess)
			{
				status = hipMalloc((void **)&d_sorted_indices,
					sorted_indices_bytes);
				if (status == hipSuccess)
				{
					/* Sort logits in descending order */
					ndb_rocm_hf_sort_logits_kernel<<<blocks,
						threads>>>(d_logits,
						d_sorted_logits,
						d_sorted_indices,
						config->vocab_size);
					...
					if (status == hipSuccess)
					{
						...
						if (status == hipSuccess)
						{
							...
							ndb_rocm_hf_top_p_filter_kernel<<<
								blocks,
								threads>>>(
								d_logits,
								d_sorted_logits,
		...
		if (gen_params->repetition_penalty > 1.0f
			&& generated_count > 0)
		{
			ndb_rocm_hf_repetition_penalty_kernel<<<blocks,
				threads>>>(d_logits,
				d_output_tokens,
				generated_count,
		...
		if (gen_params->do_sample)
		{
			...
			ndb_rocm_hf_multinomial_sample_kernel<<<blocks,
				threads>>>(d_logits,
				d_probs,
				d_cumsum,
				random_value,
		...
		{
			/* Greedy sampling */
			ndb_rocm_hf_greedy_sample_kernel<<<1, 1>>>(d_logits,
				d_output_tokens + generated_count,
				config->vocab_size);
		}
			...
			&& generated_count >= gen_params->min_tokens)
		{
			...
			for (j = 0; j < gen_params->num_stop_sequences; j++)
			{
				...
				status = hipMalloc((void **)&d_stop_seq,
					sizeof(int32_t) * stop_len);
				if (status != hipSuccess)
					continue; /* Skip this stop sequence */
				status = hipMemcpy(d_stop_seq,
				...
				ndb_rocm_hf_check_stop_sequence_kernel<<<1,
					1>>>(d_output_tokens,
					generated_count,
					d_stop_seq,
					stop_len,
	...
	*output_seq_len = generated_count;
	...
	{
		status = hipMemcpy(output_token_ids,
			d_output_tokens,
	...
	hipFree(d_output_tokens);
	hipFree(d_key_cache);
	hipFree(d_value_cache);
	hipFree(d_found_stop);
 ...
 */
__global__ static void
ndb_rocm_hf_batch_embedding_lookup_kernel(const float *embedding_table,
	const int32_t *token_ids,
	const int32_t *attention_mask,
	float *embeddings,
	int batch_size,
 ...
 */
__global__ static void
ndb_rocm_hf_extract_cls_kernel(const float *hidden_states,
	int batch_size,
	int seq_len,
	int embed_dim,
 ...
 */
__global__ static void
ndb_rocm_hf_classification_head_kernel(const float *cls_embeddings,
	const float *weights,
	float bias,
	int batch_size,
	int embed_dim,
 ...
 */
extern "C" int
ndb_rocm_hf_cross_encoder_rerank_inference(
	...
	char **errstr)
{
	...
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_cls_embeddings, cls_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_scores, score_bytes);
	if (status != hipSuccess)
		goto error;
	status = hipMalloc((void **)&d_classification_weights, sizeof(float) * embed_dim);
	if (status != hipSuccess)
		goto error;

	/* Copy input to device */
	status = hipMemcpy(d_token_ids, token_ids_batch, token_bytes, hipMemcpyHostToDevice);
	if (status != hipSuccess)
		goto error;
	status = hipMemcpy(d_attention_mask, attention_mask_batch, mask_bytes, hipMemcpyHostToDevice);
	if (status != hipSuccess)
		goto error;
	status = hipMemcpy(d_classification_weights, classification_weights,
		sizeof(float) * embed_dim, hipMemcpyHostToDevice);
	if (status != hipSuccess)
		goto error;
	...
	ndb_rocm_hf_batch_embedding_lookup_kernel<<<grid, block>>>(
		embedding_table,
		d_token_ids,
		d_attention_mask,
		d_embeddings,
	...
	status = hipMemcpy(d_hidden_states, d_embeddings, embed_bytes, hipMemcpyDeviceToDevice);
	if (status != hipSuccess)
		goto error;
	...
	ndb_rocm_hf_extract_cls_kernel<<<grid, block>>>(
		d_hidden_states,
		batch_size,
		seq_len,
		embed_dim,
	...
	ndb_rocm_hf_classification_head_kernel<<<grid, block>>>(
		d_cls_embeddings,
	...
	status = hipMemcpy(scores_out, d_scores, score_bytes, hipMemcpyDeviceToHost);
	if (status != hipSuccess)
		goto error;
