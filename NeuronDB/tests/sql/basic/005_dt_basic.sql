...
SELECT
	neurondb.train(
		...
		'label',
		ARRAY['features'],
		'{"max_depth": 10, "min_samples_split": 2}'::jsonb
	)::integer AS model_id;
...
SELECT 
	...
	ROUND(COALESCE((m.metrics::jsonb->>'precision')::numeric, 0), 6) AS precision,
	ROUND(COALESCE((m.metrics::jsonb->>'recall')::numeric, 0), 6) AS recall,
	ROUND(COALESCE((m.metrics::jsonb->>'f1_score')::numeric, 0), 6) AS f1_score
...
SELECT metrics FROM gpu_metrics_temp;
...
WHERE m.model_id = (SELECT model_id FROM test_metrics WHERE test_name = '005_dt_basic')
...
WHERE tm.test_name = '005_dt_basic';
