-- 004_svm_advance.sql
-- Exhaustive detailed test for svm: all train, error, predict, evaluate.
...
\echo '=========================================================================='
\echo 'svm: Exhaustive GPU/CPU + Error Coverage (1000 rows sample)'
\echo '=========================================================================='
...
FROM cpu_model_temp_004;

\echo 'Test 3: Custom hyperparameters (C=0.5, kernel=linear)'
DROP TABLE IF EXISTS custom_model_temp_004;
CREATE TEMP TABLE custom_model_temp_004 AS
SELECT neurondb.train('svm', 
	'test_train_view', 
	'features', 'label', 
	'{"C":0.5,"kernel":"linear"}'::jsonb)::integer AS custom_model_id;

SELECT 
	'Custom (C=0.5, linear)' AS config,
	custom_model_id AS model_id,
	(SELECT metrics->>'C' FROM neurondb.ml_models WHERE model_id = custom_model_id) AS C_value,
	(SELECT metrics->>'kernel' FROM neurondb.ml_models WHERE model_id = custom_model_id) AS kernel
FROM custom_model_temp_004;

\echo 'Test 4: Different C value (C=2.0)'
DROP TABLE IF EXISTS c2_model_temp_004;
CREATE TEMP TABLE c2_model_temp_004 AS
SELECT neurondb.train('svm', 
	'test_train_view', 
	'features', 'label', 
	'{"C":2.0}'::jsonb)::integer AS c2_model_id;
...
BEGIN
	BEGIN
		PERFORM neurondb.train('svm','missing_table','features','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('svm','test_train_view','notafeat','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('svm','test_train_view','features','notalabel','{}'::jsonb);
...
\echo 'Error Test 4: Negative C value'
DO $$
BEGIN
	BEGIN
		PERFORM neurondb.train('svm','test_train_view','features','label','{"C":-1.0}'::jsonb);
		RAISE EXCEPTION 'FAIL: expected error for negative C';
	EXCEPTION WHEN OTHERS THEN 
			NULL;
		-- Error handled correctly
		NULL;
...
BEGIN
	BEGIN
		PERFORM neurondb.train('svm','test_train_view',NULL,'label','{}'::jsonb);
...
FROM (
	SELECT neurondb.predict((SELECT gpu_model_id FROM gpu_model_temp_004 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	ROUND(MAX(score)::numeric, 4) AS max_score
FROM (
	SELECT neurondb.predict((SELECT cpu_model_id FROM cpu_model_temp_004 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	neurondb.predict((SELECT custom_model_id FROM custom_model_temp_004 LIMIT 1), features) AS prediction
FROM test_test_view
LIMIT 1;
...
SELECT 
	...
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT custom_model_id FROM custom_model_temp_004 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 100
) b;
...
BEGIN
	SELECT cpu_model_id INTO cpu_mid_temp FROM cpu_model_temp_004 LIMIT 1;
	BEGIN
...
DECLARE
	...
	cpu_mid integer;
BEGIN
	SELECT cpu_model_id INTO cpu_mid FROM cpu_model_temp_004 LIMIT 1;
	SELECT jsonb_array_length(metrics->'support_vectors') INTO model_dim
	FROM neurondb.ml_models WHERE model_id = cpu_mid
	AND metrics IS NOT NULL AND metrics->'support_vectors' IS NOT NULL;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT gpu_model_id INTO gpu_mid FROM gpu_model_temp_004 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT custom_model_id INTO custom_mid FROM custom_model_temp_004 LIMIT 1;
...
SELECT 
	...
	m.metrics->>'C' AS C_value,
	m.metrics->>'kernel' AS kernel_type,
...
FROM neurondb.ml_models m, gpu_model_temp_004 t
...
SELECT 
	...
	CASE 
		...
		ELSE format('%s bytes', pg_column_size(m.model_data))
	END AS model_data_status
FROM neurondb.ml_models m, cpu_model_temp_004 t
...
SELECT 
	...
	m.metrics->>'kernel' AS kernel_type,
	m.metrics->>'accuracy' AS accuracy
FROM neurondb.ml_models m, custom_model_temp_004 t
...
\echo ''
\echo '=========================================================================='
\echo '✓ svm: Full exhaustive code-path test complete (1000-row sample)'
\echo '=========================================================================='
