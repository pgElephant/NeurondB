-- 005_dt_advance.sql
-- Exhaustive detailed test for decision_tree: all train, error, predict, evaluate.
...
\echo '=========================================================================='
\echo 'decision_tree: Exhaustive GPU/CPU + Error Coverage (1000 rows sample)'
\echo '=========================================================================='
...
FROM cpu_model_temp_005;

\echo 'Test 3: Custom hyperparameters (max_depth=5, min_samples_split=5)'
DROP TABLE IF EXISTS custom_model_temp_005;
CREATE TEMP TABLE custom_model_temp_005 AS
SELECT neurondb.train('decision_tree', 
	'test_train_view', 
	'features', 'label', 
	'{"max_depth":5,"min_samples_split":5}'::jsonb)::integer AS custom_model_id;

SELECT 
	'Custom (depth=5, min_split=5)' AS config,
	custom_model_id AS model_id,
	(SELECT metrics->>'max_depth' FROM neurondb.ml_models WHERE model_id = custom_model_id) AS max_depth,
	(SELECT metrics->>'min_samples_split' FROM neurondb.ml_models WHERE model_id = custom_model_id) AS min_samples_split
FROM custom_model_temp_005;

\echo 'Test 4: Shallow tree (max_depth=3)'
DROP TABLE IF EXISTS shallow_model_temp_005;
CREATE TEMP TABLE shallow_model_temp_005 AS
SELECT neurondb.train('decision_tree', 
	'test_train_view', 
	'features', 'label', 
	'{"max_depth":3}'::jsonb)::integer AS shallow_model_id;

\echo 'Test 5: Deep tree (max_depth=15)'
DROP TABLE IF EXISTS deep_model_temp_005;
CREATE TEMP TABLE deep_model_temp_005 AS
SELECT neurondb.train('decision_tree', 
	'test_train_view', 
	'features', 'label', 
	'{"max_depth":15}'::jsonb)::integer AS deep_model_id;
...
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','missing_table','features','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','test_train_view','notafeat','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','test_train_view','features','notalabel','{}'::jsonb);
...
\echo 'Error Test 4: max_depth=0'
DO $$
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','test_train_view','features','label','{"max_depth":0}'::jsonb);
...
\echo 'Error Test 5: min_samples_split=0'
DO $$
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','test_train_view','features','label','{"min_samples_split":0}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('decision_tree','test_train_view',NULL,'label','{}'::jsonb);
...
FROM (
	SELECT neurondb.predict((SELECT gpu_model_id FROM gpu_model_temp_005 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	ROUND(MAX(score)::numeric, 4) AS max_score
FROM (
	SELECT neurondb.predict((SELECT cpu_model_id FROM cpu_model_temp_005 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	neurondb.predict((SELECT custom_model_id FROM custom_model_temp_005 LIMIT 1), features) AS prediction
FROM test_test_view
LIMIT 1;
...
SELECT 
	...
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT custom_model_id FROM custom_model_temp_005 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 100
) b;

\echo 'Predict Test 5: Shallow model batch (50 rows)'
SELECT 
	'Shallow Batch' AS test_type,
	COUNT(*) AS n_predictions,
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT shallow_model_id FROM shallow_model_temp_005 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 50
) b;
...
BEGIN
	SELECT cpu_model_id INTO cpu_mid_temp FROM cpu_model_temp_005 LIMIT 1;
	BEGIN
...
DECLARE
	...
	cpu_mid integer;
BEGIN
	SELECT cpu_model_id INTO cpu_mid FROM cpu_model_temp_005 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT gpu_model_id INTO gpu_mid FROM gpu_model_temp_005 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT custom_model_id INTO custom_mid FROM custom_model_temp_005 LIMIT 1;
...
\echo 'Evaluate Test 4: Shallow model evaluation'
DO $$
DECLARE
	shallow_mid integer;
	result jsonb;
BEGIN
	SELECT shallow_model_id INTO shallow_mid FROM shallow_model_temp_005 LIMIT 1;
	IF shallow_mid IS NOT NULL THEN
		BEGIN
			result := neurondb.evaluate(shallow_mid, 'test_test_view', 'features', 'label');
		EXCEPTION WHEN OTHERS THEN
		END;
	END IF;
END$$;
...
FROM neurondb.ml_models m, gpu_model_temp_005 t
...
SELECT 
	...
	CASE 
		...
		ELSE format('%s bytes', pg_column_size(m.model_data))
	END AS model_data_status
FROM neurondb.ml_models m, cpu_model_temp_005 t
...
SELECT 
	...
	m.metrics->>'min_samples_split' AS min_samples_split,
	m.metrics->>'accuracy' AS accuracy
FROM neurondb.ml_models m, custom_model_temp_005 t
...
WHERE algorithm = 'decision_tree'
GROUP BY algorithm;

\echo ''
\echo '=========================================================================='
\echo '✓ decision_tree: Full exhaustive code-path test complete (1000-row sample)'
\echo '=========================================================================='
