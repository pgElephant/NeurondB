...
-- Exhaustive detailed test for ridge: all train, error, predict, evaluate.
...
\echo '=========================================================================='
\echo 'ridge: Exhaustive GPU/CPU + Error Coverage (1000 rows sample)'
\echo '=========================================================================='
...
FROM cpu_model_temp_006;

\echo 'Test 3: Custom hyperparameters (alpha=0.5, fit_intercept=true)'
DROP TABLE IF EXISTS custom_model_temp_006;
CREATE TEMP TABLE custom_model_temp_006 AS
SELECT neurondb.train('ridge', 
	'test_train_view', 
	'features', 'label', 
	'{"alpha":0.5,"fit_intercept":true}'::jsonb)::integer AS custom_model_id;

SELECT 
	'Custom (alpha=0.5, intercept)' AS config,
	custom_model_id AS model_id,
	(SELECT metrics->>'alpha' FROM neurondb.ml_models WHERE model_id = custom_model_id) AS alpha_value
FROM custom_model_temp_006;

\echo 'Test 4: Different alpha value (alpha=2.0)'
DROP TABLE IF EXISTS alpha2_model_temp_006;
CREATE TEMP TABLE alpha2_model_temp_006 AS
SELECT neurondb.train('ridge', 
	'test_train_view', 
	'features', 'label', 
	'{"alpha":2.0}'::jsonb)::integer AS alpha2_model_id;

\echo 'Test 5: Low alpha (alpha=0.1) for weak regularization'
DROP TABLE IF EXISTS alpha01_model_temp_006;
CREATE TEMP TABLE alpha01_model_temp_006 AS
SELECT neurondb.train('ridge', 
	'test_train_view', 
	'features', 'label', 
	'{"alpha":0.1}'::jsonb)::integer AS alpha01_model_id;
...
BEGIN
	BEGIN
		PERFORM neurondb.train('ridge','missing_table','features','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('ridge','test_train_view','notafeat','label','{}'::jsonb);
...
BEGIN
	BEGIN
		PERFORM neurondb.train('ridge','test_train_view','features','notalabel','{}'::jsonb);
...
\echo 'Error Test 4: Negative alpha'
DO $$
BEGIN
	BEGIN
		PERFORM neurondb.train('ridge','test_train_view','features','label','{"alpha":-1.0}'::jsonb);
		RAISE EXCEPTION 'FAIL: expected error for negative alpha';
	EXCEPTION WHEN OTHERS THEN 
			NULL;
		-- Error handled correctly
		NULL;
...
BEGIN
	BEGIN
		PERFORM neurondb.train('ridge','test_train_view',NULL,'label','{}'::jsonb);
...
FROM (
	SELECT neurondb.predict((SELECT gpu_model_id FROM gpu_model_temp_006 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	ROUND(MAX(score)::numeric, 4) AS max_score
FROM (
	SELECT neurondb.predict((SELECT cpu_model_id FROM cpu_model_temp_006 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 1000
) sub;
...
SELECT 
	...
	neurondb.predict((SELECT custom_model_id FROM custom_model_temp_006 LIMIT 1), features) AS prediction
FROM test_test_view
LIMIT 1;
...
SELECT 
	...
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT custom_model_id FROM custom_model_temp_006 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 100
) b;

\echo 'Predict Test 5: Alpha=0.1 model batch (50 rows)'
SELECT 
	'Alpha=0.1 Batch' AS test_type,
	COUNT(*) AS n_predictions,
	ROUND(AVG(score)::numeric, 4) AS avg_score
FROM (
	SELECT neurondb.predict((SELECT alpha01_model_id FROM alpha01_model_temp_006 LIMIT 1), features) AS score
	FROM test_test_view
	LIMIT 50
) b;
...
BEGIN
	SELECT cpu_model_id INTO cpu_mid_temp FROM cpu_model_temp_006 LIMIT 1;
	BEGIN
...
DECLARE
	...
	cpu_mid integer;
BEGIN
	SELECT cpu_model_id INTO cpu_mid FROM cpu_model_temp_006 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT gpu_model_id INTO gpu_mid FROM gpu_model_temp_006 LIMIT 1;
...
DECLARE
	...
	result jsonb;
BEGIN
	SELECT custom_model_id INTO custom_mid FROM custom_model_temp_006 LIMIT 1;
...
\echo 'Evaluate Test 4: Alpha=0.1 model evaluation'
DO $$
DECLARE
	alpha01_mid integer;
	result jsonb;
BEGIN
	SELECT alpha01_model_id INTO alpha01_mid FROM alpha01_model_temp_006 LIMIT 1;
	IF alpha01_mid IS NOT NULL THEN
		BEGIN
			result := neurondb.evaluate(alpha01_mid, 'test_test_view', 'features', 'label');
		EXCEPTION WHEN OTHERS THEN
		END;
	END IF;
END$$;
...
SELECT 
	...
	m.metrics->>'alpha' AS alpha_value,
...
FROM neurondb.ml_models m, gpu_model_temp_006 t
...
SELECT 
	...
	CASE 
		...
		ELSE format('%s bytes', pg_column_size(m.model_data))
	END AS model_data_status
FROM neurondb.ml_models m, cpu_model_temp_006 t
...
SELECT 
	...
	m.metrics->>'r_squared' AS r_squared,
	m.metrics->>'mse' AS mse
FROM neurondb.ml_models m, custom_model_temp_006 t
...
WHERE algorithm = 'ridge'
GROUP BY algorithm;

\echo ''
\echo '=========================================================================='
\echo '✓ ridge: Full exhaustive code-path test complete (1000-row sample)'
\echo '=========================================================================='
