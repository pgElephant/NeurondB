...
class IdentifierParser:
	...
	def parse(self) -> IdentifierReport:
		"""Parse the pgident output"""
		# Parse pgident output (primary method)
		self._parse_pgident_output()
		
		# Only do custom analysis as supplement if pgident output is empty
		# and source files are provided for fallback validation
		if not self.issues and self.source_files:
			if self.verbose:
				print("Note: No pgident output found, performing supplemental analysis")
			self._analyze_files()
...
class IdentifierRunner:
	...
	def run_analysis(
		...
		"""Run identifier analysis using pgident tool"""
		start_time = time.time()
		
		if source_files:
			return self._analyze_files(source_files, start_time)
		elif source_dir:
			return self._analyze_directory(source_dir, extensions, start_time)
		else:
			raise ValueError("Either source_files or source_dir must be provided")
	
	def _analyze_files(self, source_files: List[str], start_time: float) -> IdentifierReport:
		"""Analyze specific files using pgident tool"""
		# Ensure pgident is available
		if not self.pgident_bin:
			self.pgident_bin = PgidentTool.find_pgident()
		
		if not self.pgident_bin or not PgidentTool.check_tool():
			raise RuntimeError(
				"pgident tool is required but not found. "
				"Please install pgident or ensure it is in your PATH. "
				"Common locations: /usr/local/bin/pgident, /usr/bin/pgident, /opt/postgresql/bin/pgident"
			)
		...
		for file_path in source_files:
			...
			# Use pgident tool for analysis
			output, returncode = PgidentTool.analyze_file(file_path, self.pgident_bin, self.verbose)
			if returncode == -1:
				# pgident tool error
				raise RuntimeError(f"Failed to run pgident on {file_path}: {output}")
	...
	def _analyze_directory(self, source_dir: str, extensions: List[str] = None, start_time: float = None) -> IdentifierReport:
		"""Analyze directory using pgident tool"""
		...
		return self._analyze_files([str(f) for f in files], start_time)
...
def main():
	...
	# Check for pgident (required)
	pgident_available = PgidentTool.check_tool()
	if not pgident_available:
		pgident_path = PgidentTool.find_pgident()
		if not pgident_path:
			print("Error: pgident tool is required but not found.")
			print("Please install pgident or ensure it is in your PATH.")
			print("Common locations: /usr/local/bin/pgident, /usr/bin/pgident, /opt/postgresql/bin/pgident")
			sys.exit(1)
